...
`sources.MonoCamera` is the capture code -- threaded grabber, drop-oldest slot,
...
def open_elp(index=0, mode=None, grayscale=True, strict=True, fourcc="MJPG", **kw):
    """
    A `sources.MonoCamera` on the ELP, with the mode checked rather than hoped.
...
def open_group(indices, mode=None, max_skew_s=None, **kw):
    """
    One index gives a `MonoCamera`, several give a `StereoCamera`.

        Returns ``(source, cameras)`` -- the second is always the flat list of
        `MonoCamera`, because per-camera counters (`n_grabbed`, `n_dropped`,
        `actual`) are not reachable through `StereoCamera` and are exactly what a
    ...
    if len(cams) == 1:
        return cams[0], cams
    return sources.StereoCamera(cams, max_skew_s=max_skew_s), cams
