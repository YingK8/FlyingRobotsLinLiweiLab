...
def _core_light(rng):
    ...
    roll = rng.random()
    if roll < 0.40:
        rig = rendermod.LightRig(dome=((rng.uniform(30, 80), rng.uniform(0, 360)),),
                                 ambient=amb, intensity=rng.uniform(6, 20))
    elif roll < 0.75:
        rig = rendermod.LightRig(lateral_deg=(rng.uniform(0, 360),),
                                 ambient=amb, intensity=rng.uniform(6, 20))
    else:
        a = float(rng.uniform(0, 360))
        spread = float(rng.uniform(140.0, 220.0))
        rig = rendermod.LightRig(lateral_deg=(a, (a + spread) % 360.0),
                                 ambient=amb, intensity=rng.uniform(5, 14))
    return rig, amb
...
def _edge_light(rng):
    ...
    if rng.random() < 0.75:
        rig = rendermod.LightRig(lateral_deg=(rng.uniform(0, 360),),
                                 ambient=amb, intensity=rng.uniform(6, 18))
    else:
        rig = rendermod.LightRig(dome=((rng.uniform(15, 40), rng.uniform(0, 360)),),
                                 ambient=amb, intensity=rng.uniform(6, 18))
    return rig, amb
...
def core(rng, n, shape, subframes=5):
    ...
    out = []
    for i in range(n):
        field, name = bgmod.sample(shape, rng, peak=bgmod.CORE_PEAK,
                                   weights=bgmod.CORE_WEIGHTS)
        light, amb = _core_light(rng)
        out.append(Condition(
            background=field, light=light,
            exposure=_exposure(rng, i, subframes),
            alpha=float(rng.choice([0.8, 0.9, 1.0])),
            tier="core", bg_name=name, ambient=amb,
        ))
    return out
...
def edge(rng, n, shape, subframes=5):
    ...
    out = []
    for i in range(n):
        field, name = bgmod.sample(shape, rng, peak=bgmod.EDGE_PEAK,
                                   weights=bgmod.EDGE_WEIGHTS)
        light, amb = _edge_light(rng)
        out.append(Condition(
            background=field, light=light,
            exposure=_exposure(rng, i, subframes, noise_scale=rng.uniform(1.0, 1.5)),
            alpha=float(rng.choice([0.7, 0.8, 0.9, 1.0])),
            tier="edge", bg_name=name, ambient=amb,
        ))
    return out
