...
Log with ai/trigger_reset_log.py, e.g.:
  uv run python ai/trigger_reset_log.py --log-seconds 65 --out current_pid_run.log
  uv run python ai/plot_pid_log.py current_pid_run.log

Also prints a machine-parsable "METRICS ..." summary line (see
ai/pid_metrics.py) that ai/pid_autotune.py scores across trials.
