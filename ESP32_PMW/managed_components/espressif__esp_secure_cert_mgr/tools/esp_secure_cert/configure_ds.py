...
supported_key_size_rsa = {
    "esp32s2": [1024, 2048, 3072, 4096],
...
supported_key_size_ecdsa = {"esp32h2": [256], "esp32p4": [256]}
...
def calculate_rsa_ds_params(privkey, priv_key_pass, hmac_key, idf_target):
    ...
    if key_size not in supported_key_size_rsa[idf_target]:
        print(
            "ERROR: Private key size {0} not supported for the target {1},"
            "\nthe supported key sizes are {2}".format(
                key_size, idf_target, str(supported_key_size_rsa[idf_target])
            )
        )
        sys.exit(-1)
...
def validate_ds_algorithm(ds_algo, key_size_bits, target_chip):
    if ds_algo == "RSA":
        if target_chip not in supported_targets_rsa_ds:
            print(f"ERROR: Target {target_chip} does not " "support RSA DS peripheral")
            sys.exit(-1)
        key_size_bits = int(key_size_bits, 10)
        if key_size_bits not in supported_key_size_rsa[target_chip]:
            print(
                f"ERROR: Target {target_chip} does not support "
        ...
        if target_chip not in supported_targets_ecdsa:
            print(f"ERROR: Target {target_chip} does not support " "ECDSA peripheral")
            sys.exit(-1)
...
def get_ecdsa_key_bytes(privkey, priv_key_pass, key_length_bytes):
    ...
    if not isinstance(private_key, ec.EllipticCurvePrivateKey):
        print(
            "ERROR: Invalid private key format." " Only ECC private keys are supported"
        )
        sys.exit(-1)
    key_length_bits = key_length_bytes * 8
    if private_key.key_size != key_length_bits:
        print(
            f"ERROR: key size {private_key.key_size} does not match"
...
def configure_efuse_for_rsa(
    ...
    if port:
        ...
        hmac_key = configure_efuse_key_block(
            idf_target, port, efuse_key_file, efuse_key_id, efuse_purpose
        )
    else:
        print(
            "INFO: No port provided. Using local key configuration (no device interaction)."
        )
        print("WARNING: The key will NOT be burned to the device eFuse automatically.")
        hmac_key = configure_efuse_key_block_local(
            efuse_key_file, efuse_key_id, efuse_purpose
        )

    return hmac_key


def configure_efuse_for_ecdsa(
    idf_target,
    port,
    ecdsa_key_file,
    ...
    try:
        ...
        if port:
            ...
            print(
                "WARNING: The ECDSA key will NOT be burned to the device eFuse automatically."
            )
            configure_efuse_key_block_local(efuse_key_file, efuse_key_id, efuse_purpose)

    except OSError:
        print(
            "Hint: For ECDSA peripheral esptool version"
