...
def _resample(image, K, width, height):
    ...
    scaled = cv2.resize(
        image, (width, int(round(image.shape[0] * s))), interpolation=cv2.INTER_AREA
    )
    K = K.copy()
    K[:2, :] *= s
...
def render_frames(n_poses, seed, subframes=5):
    ...
    tiers = {
        "core": cond.core(rng, n_poses, shape, subframes),
    ...
    print(
        f"rendering {total} pairs at {RENDER_W}x{RENDER_H} "
        f"({n_poses} poses x {len(tiers)} tiers) ...",
        flush=True,
    )
...
def score(frames, render_rig, estimator_kw=None, keep_features=False):
    ...
    for w, h, fps in MODES:
        rig = StereoRig(
            cameras=tuple(
                type(c)(
                    K=_resample(np.zeros((RENDER_H, RENDER_W), np.uint8), c.K, w, h)[1],
                    dist=c.dist,
                    T_world_cam=c.T_world_cam,
                    name=c.name,
                )
        ...
        for k, (images, ctr, nrm, tilt_deg, label) in enumerate(frames):
            small = [
                _resample(im, render_rig.cameras[j].K, w, h)[0]
        ...
        if rows:
            ...
            in_spec = float(
                np.mean((axes.max(axis=1) <= TARGET_POS_MM) & (ang <= TARGET_ANGLE_DEG))
            )
            rec = {
                "width": w,
                "height": h,
                ...
                "ang_p95": float(np.percentile(ang, 95)),
        ...
        out.append(rec)
        print(
            f"    {w}x{h} @{fps}: n={len(rows)}/{len(frames)} "
...
def chart(tiers, path, title=""):
    ...
    for tier, records in tiers.items():
        ...
        (h1,) = ax.plot(
            ...
            "o-",
            color="#2A8FC4",
            lw=2.2 if solid else 1.4,
            ms=5 if solid else 3.5,
            alpha=alpha,
            label=f"angle mean ({tier})",
        )
        (h2,) = ax.plot(
            x,
            angm,
        ...
        bottom=max(
            ...
            min(
                [
                    ...
                    for k in ("ang_mean", "ang_max")
                    for v in [r_i.get(k) for r_i in r]
                    if v
                ]
                + [1.0]
            )
...
def main(argv=None):
    ...
    for tier in ("core", "edge"):
        ...
        if args.save_samples:
            tiers[tier], s_t = score(
                frames[tier], render_rig, estimator_kw=kw, keep_features=True
            )
            for row in s_t:
                row["tier"] = tier
            samples += s_t
    ...
    chart(
        tiers,
        RESULTS / f"resolution_{args.tag}.png",
