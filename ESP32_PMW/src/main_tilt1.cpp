...
// All channels start at 100% so disk spins up at full power
...
// --- STATE MACHINE: sweep B (ch1) and D (ch3) from 100% down to 50% ---
// First step fires at 90% (100 - 10), last at 50% (60 > 50 condition).
...
void loop() {
  ...
  // Sweep B and D from 100% down to 50%; stop below 50%
  if (ramp_finished && carrier_duty_sweep > 50.0f) {

    if (current_millis - wait_start_time >= wait_time_ms) {
      carrier_duty_sweep -= d_duty_step;

      if (carrier_duty_sweep < 50.0f) {
        carrier_duty_sweep = 50.0f;
      }

      controller->setCarrierDutyCycle(1, carrier_duty_sweep); // B
      controller->setCarrierDutyCycle(3, carrier_duty_sweep); // D
