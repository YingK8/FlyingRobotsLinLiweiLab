...
Run: uv run python controller/pose/test_stereo.py
"""
...
def _hulls(rig, center_world, normal_world, radius=RADIUS_MM, n=48, noise_px=0.0, seed=0):
    ...
    for cam in rig.cameras:
        c_cam, n_cam = cam.to_camera(center_world, normal_world)
        (cx, cy), (major, minor), ang = conic.project_circle(c_cam, n_cam, radius, cam.K)
...
def test_forward_backward_roundtrip():
    ...
    for ctr, nrm in TRUTH:
        ...
        for cam, cands in zip(rig.cameras, _candidates(rig, ctr, nrm)):
            assert len(cands) >= 1, "back-projection returned nothing for an exact ellipse"
            errs = []
            for c in cands:
...
def test_match_picks_the_true_branch():
    ...
    print(f"  branch match        exact on {len(TRUTH)}/{len(TRUTH)}; "
...
def test_margin_collapses_when_it_should():
    ...
    print(f"  margin vs tilt      face-on {flat.margin:.1f}, tilted {tilted.margin:.1f} "
          f"(sigma^2 units)")
...
def test_fusion_beats_either_view_on_depth():
    ...
    for cam, cands in zip(rig.cameras, _candidates(rig, ctr, nrm)):
        true_pose = min(
            cands, key=lambda c: np.linalg.norm(cam.to_world(c.center, c.normal)[0] - ctr)
        )
        # Push it away along the camera's own view direction.
        bad_world = cam.to_world(true_pose.center, true_pose.normal)[0] \
    ...
    assert err < 0.25 * min(solo), (
        f"fusion only improved {min(solo) / max(err, 1e-9):.1f}x; expected a large factor"
    )
    print(f"  fusion on depth     {min(solo):.3f}/{max(solo):.3f} mm single -> "
...
def test_fused_covariance_matches_the_prediction():
    ...
    poses = []
    for cam, cands in zip(rig.cameras, _candidates(rig, ctr, nrm)):
        poses.append(min(cands, key=lambda c: np.linalg.norm(
            cam.to_world(c.center, c.normal)[0] - ctr)))
    _, _, cov = stereo.fuse(poses, rig)
    want = rig.position_covariance(stereo.SIGMA_LAT_MM, stereo.SIGMA_DEPTH_MM)
    assert np.allclose(cov, want, rtol=1e-9), "fuse and rig.position_covariance disagree"
    sig = np.sqrt(np.linalg.eigvalsh(cov))
    print(f"  covariance          per-axis sigma {np.array2string(sig, precision=4)} mm")
...
def test_refine_recovers_exact_geometry():
    ...
    for ctr, nrm in TRUTH:
        ...
        r = stereo.refine(hulls, rig, ctr + np.array([3.0, -2.0, 2.0]), bad_n,
                          RADIUS_MM, params="both", xtol=1e-12, ftol=1e-12, gtol=1e-12)
        assert r is not None, "five-parameter refinement returned nothing on a clean view"
        ...
        worst["normal"][1] = max(worst["normal"][1], stereo.line_angle_deg(r.normal, nrm))
    ...
    assert worst["normal"][1] < 1e-2, f"normal-only refine off by {worst['normal'][1]:.3e} deg"
    print(f"  refine (clean)      both: {worst['both'][0]:.2e} mm / {worst['both'][1]:.2e} deg "
...
def test_refine_is_harmless_without_model_error():
    ...
    assert f < 0.25, f"refined normal {f:.4f} deg is far worse than expected on ideal data"
    assert f <= s * 2.0, f"refinement degraded the normal badly: {s:.4f} -> {f:.4f} deg"
    print(f"  refine (ideal)      seed {s:.4f} -> refined {f:.4f} deg; nothing to correct")
...
def test_refine_removes_silhouette_model_error():
    ...
    for trial in range(24):
        ...
        for cam in rig.cameras:
            ...
            hulls.append(_ellipse_points(cal.unapply(conic.normalise_ellipse(ideal)),
                                         rng, noise_px=0.2))
        seed = _seed_from_hulls(rig, hulls)
        if seed is None:
            continue
    ...
    print(f"  refine (mast model)  seed {s:.4f} deg -> refined {f:.4f} deg "
...
def test_normals_are_lines_across_hemispheres():
    ...
    assert float(fused_n @ [0, 0, 1]) > 0, "fused normal was not oriented against the reference"
...
def test_speed():
    ...
    print(f"  speed               match {t_match * 1e3:.0f} us, fuse {t_fuse * 1e3:.0f} us, "
