...
// Sync-period glitch floor: below the shortest period this rig ever drives
// (~4762us at 210Hz), so a real edge always exceeds it. Anything shorter is
// noise (contact bounce, ISR re-entry), not a genuine pulse. Must shrink if
// the drive frequency range ever goes above ~500Hz.
const int64_t MIN_VALID_SYNC_DELTA_US = 2000;
...
namespace
{
                                      ...
                                      uint8_t &selectedBits)
    {
        ...
        if (startBits > 1)
            startBits--; // Back off to last valid estimate.
...
PWMController::PWMController(const gpio_num_t *pins, const float *phaseOffsetsDegrees, const float *dutyCycles, int numChannels)
{
    ...
    _syncPin = GPIO_NUM_NC; // safe default: no garbage GPIO access before enableSync()
...
PWMController::~PWMController()
{
    ...
    if (_carrierPinsArray)
        delete[] _carrierPinsArray;
    if (_carrierDutyCyclePct)
        delete[] _carrierDutyCyclePct;
    if (_carrierLedcConfigured)
        delete[] _carrierLedcConfigured;
    if (_carrierLastDutyTicks)
        delete[] _carrierLastDutyTicks;
}
...
void PWMController::begin(float initialFreqHz)
{
    for (int i = 0; i < _numChannels; i++)
    {
        ...
        // Idle HIGH: the inverter sends the H-bridge LOW, protecting it from
        // phantom-powering via ESD diodes.
...
// COMPILE-TIME OPTIMIZED SETUP
    ...
    const esp_timer_create_args_t timer_args = {
        ...
        .arg = this,
        .dispatch_method = ESP_TIMER_TASK, // task context, not ISR: avoids RTOS starvation
        .name = "pwm_gen"};
...
void IRAM_ATTR PWMController::_timerCallback(void *arg)
{
    ...
    // Runs in task context (dispatch_method=ESP_TIMER_TASK), so use
    // portENTER_CRITICAL, not the _ISR variant.
...
// Client fallback: If waiting for first sync, use default period
    ...
    // 32-bit math from here: a 64-bit divide would pull division routines
    // from flash mid-interrupt and crash.
    int64_t delta = now - lastSync;
    if (delta < 0)
        delta += period; // Handle case if sync pushed lastSync slightly into the future
...
// Channel Generation
    ...
    for (int i = 0; i < channelLimit; i++)
    {
        ...
        if (self->_pins[i] == GPIO_NUM_NC || self->_pins[i] > GPIO_NUM_39)
            continue;
        ...
        bool active = self->_params[i].wraps ? (timeInCycle >= start || timeInCycle < end) : (timeInCycle >= start && timeInCycle < end);

        // Active-low: the NC7SZ04P5X inverter means LOW here drives the
        // H-bridge ON, HIGH drives it OFF.
...
void IRAM_ATTR PWMController::_onSyncInterrupt()
{
    ...
    {
        int64_t timeSinceLastEdgeUs = now - _isrInstance->_lastIsrTimeUs;

        if (timeSinceLastEdgeUs > MIN_VALID_SYNC_DELTA_US)
        {
            int64_t priorAvgPeriodUs = _isrInstance->_averagedPeriodUs;
            int64_t periodSampleUs = timeSinceLastEdgeUs;

            // One dropped edge doubles the gap between edges: recover the
            // true period by halving it, instead of feeding a 2x outlier
            // into the average.
            if (priorAvgPeriodUs > 0 &&
                abs(timeSinceLastEdgeUs - 2 * priorAvgPeriodUs) < (priorAvgPeriodUs / 2))
            {
                periodSampleUs = timeSinceLastEdgeUs / 2;
            }

            _isrInstance->_periodBuffer[_isrInstance->_filterIdx] = periodSampleUs;
            _isrInstance->_filterIdx = (_isrInstance->_filterIdx + 1) % FREQ_FILTER_SIZE;

            int64_t periodSampleSumUs = 0;
            for (int i = 0; i < FREQ_FILTER_SIZE; i++)
                periodSampleSumUs += _isrInstance->_periodBuffer[i];
            _isrInstance->_averagedPeriodUs = periodSampleSumUs / FREQ_FILTER_SIZE;
        }
    }

    _isrInstance->_lastIsrTimeUs = now;

    // Compensate for fixed ISR dispatch latency.
...
void PWMController::setGlobalFrequency(float newHz)
{
    ...
    // Rescale position-in-cycle to the new period so a frequency change
    // doesn't jump phase.
    if (_averagedPeriodUs > 0)
    {
        int64_t oldPos = (now - _lastSyncTimeUs) % _averagedPeriodUs;
        if (oldPos < 0)
            oldPos += _averagedPeriodUs;
...
{
    if (!_carrierDutyCyclePct || channel < 0 || channel >= _numChannels)
        return 0.0f;
    return _carrierDutyCyclePct[channel];
}

void PWMController::step()
{
...
{
    if (!pins || !dutyPercents || freqHz <= 0.0f)
        return;
    ...
    for (int i = 0; i < _numChannels; i++)
    {
        ...
        if (pin == GPIO_NUM_NC || pin > GPIO_NUM_39)
            continue;
        ...
        ledc_channel_config_t ledc_channel = {
            ...
            .hpoint = 0,
            .flags = {.output_invert = 0} // set to 1 for LEDC to invert in hardware
        };
...
void PWMController::setCarrierDutyCycle(int channel, float dutyPercent)
{
    if (channel < 0 || channel >= _numChannels)
        return;
    if (!_carrierPinsArray || !_carrierDutyCyclePct)
        return;
    if (_carrierPinsArray[channel] == GPIO_NUM_NC)
        return;
    if (_carrierFreqHz <= 0.0f)
        return;
    ...
    if (dutyValue == _carrierLastDutyTicks[channel])
        return;
...
bool PWMController::rampDownStep(float stepPct)
{
    if (!_carrierDutyCyclePct)
        return true;
    if (stepPct <= 0.0f)
        stepPct = 0.1f; // guard: must make progress
    bool allZero = true;
    for (int i = 0; i < _numChannels; i++)
    {
