...
def analyze_one(seg_time_fn, fw2scope, label=""):
    ...
    for i, j, seg_key in PAIR_KEYS:
        ...
        typ = (
            "reactive (same-sign)" if si * sj > 0 else "REAL-POWER transfer (opp-sign)"
        )
        print(
            f"{i+'+'+j:>6}{dphi:>5.0f}° | {i}:{si*100:+5.1f}%  {j}:{sj*100:+5.1f}%   | {typ}"
        )
    ...
    print(
        "ALL segment (mV):",
        {k: round(v) for k, v in allc.items()},
        " -> A change vs solo:",
        f"{allc['A']/solo['A']-1:+.0%}",
    )
    ...
    for a, c in enumerate(coils):
        print(
            f"  {c}  "
            + "".join(
                f"{M[a,b]:7.1f}" if not np.isnan(M[a, b]) else f"{'-':>7}"
                for b in range(4)
            )
        )
    print()
...
def plot_matrix(M, coup, out_path, title_suffix=""):
    ...
    ax1.set_xticklabels(coils)
    ax1.set_yticks(range(4))
    ax1.set_yticklabels(coils)
    for a in range(4):
        for b in range(4):
...
if args.segments_log:
    ...
    for level in sorted(by_level):
        ...
        if fw2scope is None:  # detect once, from the lowest current level present
            ...
            print(
                "detected firmware->scope map:", {k: v[-1] for k, v in fw2scope.items()}
            )
            print()
        ...
        plot_matrix(
            M, coup, out, title_suffix=f" @ {level}% ({args.direction.upper()})"
        )
