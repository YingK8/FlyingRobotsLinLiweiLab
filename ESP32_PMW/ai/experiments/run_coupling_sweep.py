...
ai/trigger_reset_log.py's EN-pulse reset + labeled-telemetry log a few
...
reset+log mode is enough (unlike ai/pid_autotune.py, which does send
commands). Regenerate the JSON files first with ai/gen_coupling_experiment.py
for different current levels/timing.

Usage:
  uv run python ai/run_coupling_sweep.py [--port /dev/ttyUSB0]
...
def compute_total_s(json_path: str) -> float:
    ...
    with open(json_path) as f:
        events = json.load(f)
    wait_ms = sum(
        e.get("duration_ms", 0) for e in events if e.get("method") == "addWaitTask"
    )
...
def flash_experiment(pio: str, direction: str) -> None:
    ...
    if not os.path.exists(src):
        raise SystemExit(f"missing {src} -- run ai/gen_coupling_experiment.py first")
...
def record_direction(direction: str, total_s: float, port, out_dir: str):
    ...
    scope_cmd = [
        sys.executable,
        os.path.join(REPO_ROOT, "tools", "picoscope_record.py"),
        "--seconds",
        str(record_s),
        "--out",
        csv_path,
    ...
    trigger_cmd = [
        sys.executable,
        os.path.join(REPO_ROOT, "tools", "trigger_reset_log.py"),
        "--delay",
        "4",
        "--log-seconds",
        str(record_s),
...
def main() -> None:
    ...
    ap.add_argument(
        "--direction", nargs="+", default=["cw", "ccw"], choices=["cw", "ccw"]
    )
    ap.add_argument(
        "--out-dir",
        default=None,
    ...
    if not args.assume_coils_off:
        resp = input(
            "Flashing resets the board through the bootloader -- "
            "confirm the coil supply is OFF before continuing [y/N]: "
        )
        if resp.strip().lower() != "y":
            raise SystemExit(
                "aborted -- turn the coil supply off, then re-run "
    ...
    for direction in args.direction:
        ...
        print(
            f"== {direction.upper()}: experiment ~{total_s:.1f}s, "
            f"recording {total_s + MARGIN_S:.1f}s =="
        )
        flash_experiment(args.pio, direction)
        input(
            "Firmware flashed -- power the coils now, then press Enter to "
            "start recording."
        )
    ...
    for direction, csv_path, log_path in results:
        print(
            f"  uv run python ai/coupling_matrix.py {csv_path} "
