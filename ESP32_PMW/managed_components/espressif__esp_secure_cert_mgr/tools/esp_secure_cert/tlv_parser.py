...
def get_padding_length(length: int) -> int:
    ...
    return (
        MIN_ALIGNMENT_REQUIRED - (length % MIN_ALIGNMENT_REQUIRED)
...
def verify_tlv_integrity(
    tlv_header: Dict, tlv_data: bytes, tlv_footer: Dict
) -> Tuple[bool, int]:
    """
...
def parse_tlv_footer(data: bytes, offset: int) -> Tuple[Dict, int]:
    ...
    tlv_footer = {"crc": tlv_footer_data[0]}
...
def format_tlv_data(tlv_type: int, tlv_flags: int, tlv_data: bytes) -> str:
    ...
    if (
        tlv_type == tlv_type_t.ESP_SECURE_CERT_CA_CERT_TLV
        or tlv_type == tlv_type_t.ESP_SECURE_CERT_DEV_CERT_TLV
    ):
        ...
        tlv_type >= tlv_type_t.ESP_SECURE_CERT_USER_DATA_1_TLV
        and tlv_type <= tlv_type_t.ESP_SECURE_CERT_USER_DATA_5_TLV
    ):
...
def print_partition_summary(tlv_entries: list):
    ...
    print("\nKey Configuration:")
    has_ds_data = any(
        e["header"]["type"] == tlv_type_t.ESP_SECURE_CERT_DS_DATA_TLV
        for e in tlv_entries
    )
    has_ds_context = any(
        e["header"]["type"] == tlv_type_t.ESP_SECURE_CERT_DS_CONTEXT_TLV
        for e in tlv_entries
    )
    has_sec_cfg = any(
        e["header"]["type"] == tlv_type_t.ESP_SECURE_CERT_SEC_CFG_TLV
        for e in tlv_entries
    )
    has_priv_key = any(
        e["header"]["type"] == tlv_type_t.ESP_SECURE_CERT_PRIV_KEY_TLV
        for e in tlv_entries
    )
...
def process_tlv_entries(file_path: str):
    ...
    while offset < len(data):
        try:
            ...
            is_valid, calculated_crc = verify_tlv_integrity(
                tlv_header, data[offset:], tlv_footer
            )
            ...
            print(
                f"CRC32:      0x{tlv_footer['crc']:08X} ({'✓ VALID' if is_valid else '✗ INVALID'})"
            )
            ...
            if tlv_header["length"] > 0:
                print("Data:")
                formatted_data = format_tlv_data(
                    tlv_type, tlv_header["flags"], tlv_data
                )
