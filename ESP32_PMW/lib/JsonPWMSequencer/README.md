...
(`channel` defaults to "no channel", i.e. an empty list -- see below).
Unknown `method` strings are logged as a warning at load time but don't
abort the rest of the file.

`channel` accepts either a single int (`"channel": 2`) or a list
(`"channel": [0, 2, 3]`) for any method that takes one -- a list applies the
same `value`/`from`/`to` to every listed channel in a single task, instead
of needing one task per channel. An out-of-range channel (or an empty list)
makes the whole task invalid, logged the same as an unknown `method`.

`method` is one of:

| method | fields used | effect |
|---|---|---|
| `addDutyCycleTask` | `channel`, `value` | instantly set one or more channels' commutation duty (0-100%) |
| `addPhaseTask` | `channel`, `value` | instantly set one or more channels' phase (degrees) |
| `addCarrierDutyCycleTask` | `channel`, `value` | instantly set one or more channels' carrier duty (0-100%) |
...
| `addPhaseRampTask` | `channel`, `from`, `to`, `duration_ms` | ramp one or more channels' phase, others unchanged |
...
| `activateChannels` | `mask` (0-15 bitmask), `value` (ON carrier duty %) | instantly set carrier duty to `value` for masked channels, `0` for the rest -- a bitmask alternative to `addCarrierDutyCycleTask`'s `channel` list when you also want the *unlisted* channels forced to 0 in the same task |
...
[
  ...
  { "method": "label", "value": "TILT_HOLD_100PCT" },
  { "method": "addCarrierDutyCycleTask", "channel": [0, 1, 2, 3], "value": 100.0 },
  { "method": "addWaitTask", "duration_ms": 2500 }
]
```
...
seq.start();
// in loop(): seq.step();  isDone() reports queue exhaustion.
```
