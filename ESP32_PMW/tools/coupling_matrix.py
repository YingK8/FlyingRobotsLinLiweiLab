...
(4 solos, 6 pairs, ALL; see main_experiment.cpp + tools/gen_coupling_experiment.py,
...
Two timing modes:
  ...
  --segments-log LOG: parse main_experiment.cpp's "t=.. label=.." telemetry
    (as captured by tools/trigger_reset_log.py) for per-segment timing
...
Usage:
  uv run python tools/coupling_matrix.py CSV [--t0 SEC] [--direction cw|ccw]
  uv run python tools/coupling_matrix.py CSV --segments-log LOG [--direction cw|ccw]
"""
import argparse
import re
...
ap.add_argument("--segments-log", default=None,
                 help="serial log with main_experiment.cpp's 't=.. label=..' telemetry "
                      "(captured via tools/trigger_reset_log.py) -- use labeled segment "
...
ap.add_argument("--reset-delay", type=float, default=4.0,
                 help="seconds between picoscope_record.py starting and the EN-pulse "
                      "reset (matches tools/run_coupling_sweep.py's --delay) -- offsets "
