...
def test_ptr_additional_records_for_service(dig_app):
    ...
    dig_app.check_additional(
        resp, "SRV", "test_service._http._tcp.local", expected=True
    )
    dig_app.check_additional(
        resp, "TXT", "test_service._http._tcp.local", expected=True
    )
...
def check_txt_for_service(
    instance, service, proto, mdns_console, dig_app, host=None, with_inst=False
):
    for_host_arg = f"-h {host}" if host is not None else ""
    for_inst_arg = f"-i {instance}" if with_inst else ""
    mdns_console.send_input(
        f"mdns_service_txt_set {service} {proto} {for_host_arg} {for_inst_arg} key1 value1"
    )
    dig_app.check_record(
        f"{instance}.{service}.{proto}.local", query_type="SRV", expected=True
    )
    dig_app.check_record(
        f"{service}.{proto}.local",
        query_type="TXT",
        expected=True,
        expect="key1=value1",
    )
    mdns_console.send_input(
        f"mdns_service_txt_set {service} {proto} {for_host_arg} {for_inst_arg} key2 value2"
    )
    dig_app.check_record(
        f"{service}.{proto}.local",
        query_type="TXT",
    ...
    mdns_console.send_input(
        f"mdns_service_txt_remove {service} {proto} {for_host_arg} {for_inst_arg} key2"
    )
    dig_app.check_record(
        f"{service}.{proto}.local",
        query_type="TXT",
    ...
    mdns_console.send_input(
        f"mdns_service_txt_replace {service} {proto} {for_host_arg}  {for_inst_arg} key3=value3 key4=value4"
    )
    dig_app.check_record(
        f"{service}.{proto}.local",
        query_type="TXT",
...
def test_update_txt(mdns_console, dig_app):
    check_txt_for_service(
        "local", "_test", "_tcp", mdns_console=mdns_console, dig_app=dig_app
    )
    check_txt_for_service(
        "local",
        "_test",
        "_tcp",
        mdns_console=mdns_console,
        dig_app=dig_app,
        with_inst=True,
    )
...
def test_service_port_set(mdns_console, dig_app):
    dig_app.check_record(
        "local._test._tcp.local", query_type="SRV", expected=True, expect="80"
    )
    mdns_console.send_input("mdns_service_port_set _test _tcp 81")
    dig_app.check_record(
        "local._test._tcp.local", query_type="SRV", expected=True, expect="81"
    )
    mdns_console.send_input("mdns_service_port_set _test2 _tcp -h delegated 82")
    dig_app.check_record(
        "extern._test2._tcp.local", query_type="SRV", expected=True, expect="82"
    )
    mdns_console.send_input(
        "mdns_service_port_set _test2 _tcp -h delegated -i extern 83"
    )
    dig_app.check_record(
        "extern._test2._tcp.local", query_type="SRV", expected=True, expect="83"
    )
    mdns_console.send_input(
        "mdns_service_port_set _test2 _tcp -h delegated -i invalid_inst 84"
    )
...
def test_service_subtype(mdns_console, dig_app):
    ...
    dig_app.check_record(
        "_subtest._sub._test._tcp.local", query_type="PTR", expected=True
    )
    mdns_console.send_input(
        "mdns_service_subtype _test2 _tcp _subtest2 -i extern -h delegated"
    )
    dig_app.check_record(
        "_subtest2._sub._test2._tcp.local", query_type="PTR", expected=True
    )
...
def test_service_set_instance(mdns_console, dig_app):
    ...
    mdns_console.send_input(
        "mdns_service_instance_set _test2 _tcp extern2 -h delegated"
    )
    mdns_console.send_input("mdns_service_lookup _test2 _tcp -d")
    mdns_console.get_output("PTR : extern2")
    dig_app.check_record("extern2._test2._tcp.local", query_type="SRV", expected=True)
    mdns_console.send_input(
        "mdns_service_instance_set _test2 _tcp extern3 -h delegated -i extern"
    )
    mdns_console.get_output("ESP_ERR_NOT_FOUND")
