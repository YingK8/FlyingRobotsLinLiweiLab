...
class ZTracker:
    ...
    def __init__(self, times: np.ndarray, heights: np.ndarray,
                 limits: TorqueLimits | None = None, tau_zdot: float = 0.08):
    ...
    def step(self, t: float, z: float, dt: float, z_dot: float | None = None) -> float:
        ...
        zdot_ref = (z_ref(t + dt, self.times, self.heights)
                    - z_ref(t - dt, self.times, self.heights)) / (2.0 * dt)
        a_des = (self.kp * error + self.kv * (zdot_ref - zdot)
...
def demo() -> None:
    ...
    print(f"f_resonance = {1.0 / (2 * math.pi * math.sqrt(INDUCTANCE_H * CAPACITANCE_F)):.1f} Hz")
    ...
    for i in range(90):
        ...
        a = lim.g * ((f / lim.f_hover) ** 2 - 1.0)
        zdot, z = zdot + a * dt, z + zdot * dt
    print(f"saturated:  f_max = {f_max:.2f} Hz vs f_ceiling {f_ceil:.2f} Hz, "
