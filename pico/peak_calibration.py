...
R = {"Channel A": 2532.0, "Channel B": 2530.0, "Channel C": 2543.0, "Channel D": 2540.0}
...
def mode_A():
    ...
    df = pd.read_csv(RAMP, skiprows=[1])
    t = df["Time"].values
    print("MODE A — two-point PEAK calibration (probe @100% duty + ramp offset)\n")
    print(
        f"{'ch':3}{'offset mV':>10}{'Vcs_pk V':>10}{'I_pk(A)':>9}{'g_peak A/V':>12}{'K_eff':>8}"
    )
    any_done = False
    for ch in CH:
        offset = float(np.median(df[ch].values[t < 1.0]))
        vpk = float(
            np.median(peak_env(df[ch].values)[t > t.max() - 2.0])
        )  # plateau peak
        ipk = PROBE_PEAK_100[ch]
        if ipk is None:
            print(
                f"{ch[-1]:3}{offset*1000:10.1f}{vpk:10.4f}{'--':>9}{'(fill probe)':>12}{'':>8}"
            )
            continue
        g = ipk / (vpk - offset)
        print(
            f"{ch[-1]:3}{offset*1000:10.1f}{vpk:10.4f}{ipk:9.2f}{g:12.2f}{R[ch]*g:8.0f}"
        )
        any_done = True
    if not any_done:
        print(
        ...
        print(
            ">> series current probe at 100% duty for each channel and fill PROBE_PEAK_100."
        )
...
def mode_B():
    ...
    try:
        ...
        print(
            "while ramping duty 0->100%. Then re-run: python3 peak_calibration.py sweep"
        )
        return

    # expect columns: Time, CS, Probe (case-insensitive, first matching)
    def col(key):
        return next(c for c in raw.columns if key in c.lower())

    cs = raw[col("cs")].values
    ip = raw[col("probe")].values
    cs_pk = peak_env(cs)
    ip_pk = peak_env(ip)
    ...
    ax.plot(
        xx, a * xx + b, "k-", lw=1.5, label=f"Vcs = {offset*1000:.0f}mV + I/{g:.2f}"
    )
    ax.set_xlabel("Probe PEAK current (A)")
    ax.set_ylabel("CS PEAK envelope (V)")
    ax.set_title(f"Peak CS calibration (R^2={r2:.3f})")
    ax.grid(alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig("peak_calibration_fit.png", dpi=140)
