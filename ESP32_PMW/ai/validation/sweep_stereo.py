...
def _mono_oracle(seg, cam, truth_c, truth_n, radius_mm, tilt_cal):
    ...
    for c in cands:
        ...
        scored.append(
            (
                float(np.linalg.norm(cw - truth_c)),
                stereomod.line_angle_deg(nw, truth_n),
                cw,
            )
        )
    ...
    return (best[2] - truth_c, best[1], branch_wrong, conic.ambiguity_margin_deg(cands))
    ...
    if True:
        for i in range(n_poses):
            ...
            row = {
                ...
                "cx_mm": s.center_world[0],
                "cy_mm": s.center_world[1],
            ...
            if pose is not None:
                ...
                row.update(
                    {
                        ...
                        "stereo_normal_deg": stereomod.line_angle_deg(
                            pose.normal, s.normal_world
                        ),
                        "discrepancy_mm": pose.discrepancy_mm,
                        "margin_mm": pose.margin_mm,
                        "refine_rms_px": pose.refine_rms_px,
                        "refine_iters": pose.refine_iters,
                ...
                row["seed_normal_deg"] = stereomod.line_angle_deg(
                    seed_n, s.normal_world
                )
            ...
            if progress and (i + 1) % 50 == 0:
                ...
                print(
                    f"    {i + 1}/{n_poses}  {el:5.1f}s elapsed, "
                    f"{el / (i + 1) * (n_poses - i - 1):5.1f}s left",
                    flush=True,
                )
    return rows
...
def summarise(rows, label=""):
    ...
    def col(name):
        return np.array(
            [
                r[name]
                for r in det
                if r.get(name) is not None and np.isfinite(r.get(name, np.nan))
            ]
        )
    ...
    print(
        f"    {'axis':<10} {'median':>9} {'p95':>9} {'max':>9}   gate {TARGET_AXIS_MM} mm"
    )
    worst_median = 0.0
    for ax in ("dx", "dy", "dz"):
        ...
        print(
            f"    {ax:<10} {np.median(v):9.4f} {np.percentile(v, 95):9.4f} "
            f"{v.max():9.4f}   {flag}"
        )
    ...
    for name, key in (("|pos|", "stereo_pos_mm"), ("normal deg", "stereo_normal_deg")):
        ...
        if len(v):
            print(
                f"    {name:<10} {np.median(v):9.4f} {np.percentile(v, 95):9.4f} "
                f"{v.max():9.4f}"
            )
    ...
    if len(mono_p):
        ...
        print(
            f"    vs mono (oracle branch):  |pos| {np.median(mono_p):.4f} -> "
        ...
        if len(bw):
            print(
                f"    mono would have picked the wrong branch on "
    ...
    if len(sd):
        print(
            f"    closed-form seed -> refined:  |pos| {np.median(sd):.4f} -> "
    ...
    if len(disc):
        print(
            f"    cross-view discrepancy {np.median(disc):.4f} mm median, "
    ...
    if len(ts):
        ...
        print(
            f"    compute: seg {np.median(ts):.3f} + solve {np.median(te):.3f} = "
...
def main(argv=None):
    ...
    ap.add_argument(
        "--poses", type=int, default=250, help="poses per rig configuration"
    )
    ap.add_argument(
        "--quick", action="store_true", help="40 poses, one rig, for a smoke test"
    )
    ap.add_argument(
        "--elev",
        type=float,
    ...
    ap.add_argument(
        "--loss", default="linear", choices=["soft_l1", "linear", "huber", "cauchy"]
    )
    ...
    print(
        f"stereo sweep: {len(configs)} configuration(s) x {n} poses at "
    ...
    for elev, azim, occ in configs:
        rig = StereoRig.from_spherical(
            elev_deg=elev, azim_deg=azim, range_mm=args.range_mm
        ).scaled(scale)
        label = (
            f"elev {elev[0]:+.0f}/{elev[1]:+.0f}  sep {rig.axis_separation_deg():.0f}deg"
            f"  tilt seen {rig.tilt_seen_deg()[0]:.0f}deg" + ("  +stand" if occ else "")
        )
    ...
    if len(summaries) > 1:
        ...
        for s in sorted(summaries, key=lambda s: s["worst_axis_median"]):
            print(
                f"  {s['label']:<44} {s['worst_axis_median']:9.4f}mm "
