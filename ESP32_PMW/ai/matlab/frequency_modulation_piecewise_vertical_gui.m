function frequency_modulation_piecewise_vertical_gui
% FREQUENCY_MODULATION_PIECEWISE_VERTICAL_GUI
...
%
% Vertical model (upward positive, no vertical drag/contact):
%   Lift = mg*(f_robot/f_hover)^2
...
fig = figure( ...
    'Name', 'Flying Robot Piecewise Frequency and Vertical Motion GUI', ...
    'NumberTitle', 'off', ...
    'MenuBar', 'none', ...
    'ToolBar', 'none', ...
...
marginTip = 'Torque headroom M = max magnetic torque / drag at the first Start freq. M>=1. Step-out ceiling f_max = f0*sqrt(M).';
uicontrol('Parent', settingsPanel, 'Style', 'text', ...
    'String', 'Torque margin', 'HorizontalAlignment', 'left', ...
    'TooltipString', marginTip, ...
...
autoChainCheck = uicontrol('Parent', settingsPanel, 'Style', 'checkbox', ...
    ...
    'Units', 'normalized', ...
    'Position', [0.600 0.690 0.370 0.200], ...
    'Callback', @autoChainChanged);

velTip = 'Initial vertical velocity at t=0, m/s, upward positive.';
uicontrol('Parent', settingsPanel, 'Style', 'text', ...
    'String', 'Initial vertical velocity (m/s)', ...
    'HorizontalAlignment', 'left', ...
    'TooltipString', velTip, ...
    'Units', 'normalized', 'Position', [0.020 0.395 0.255 0.180]);
initialVelocityEdit = uicontrol('Parent', settingsPanel, 'Style', 'edit', ...
    'String', '0', 'BackgroundColor', 'white', ...
    'TooltipString', velTip, ...
    'Units', 'normalized', 'Position', [0.270 0.420 0.105 0.180]);

hoverTip = 'Spin frequency where lift = weight (hover). L/mg = (f_robot/f_hover)^2; z_ddot = g*((f_robot/f_hover)^2 - 1).';
uicontrol('Parent', settingsPanel, 'Style', 'text', ...
    'String', 'Lift = weight frequency (Hz)', ...
    'HorizontalAlignment', 'left', ...
    'TooltipString', hoverTip, ...
    'Units', 'normalized', 'Position', [0.405 0.395 0.245 0.180]);
hoverFrequencyEdit = uicontrol('Parent', settingsPanel, 'Style', 'edit', ...
    'String', '140', 'BackgroundColor', 'white', ...
    'TooltipString', hoverTip, ...
...
uicontrol('Parent', settingsPanel, 'Style', 'text', ...
    'String', ['Vertical model uses the simulated robot frequency: ' ...
        'L/mg = (f_{robot}/f_{hover})^2. Upward is positive; vertical drag and contact are ignored.'], ...
    'FontSize', 8.5, ...
    'ForegroundColor', [0.35 0.35 0.35], ...
    'HorizontalAlignment', 'left', ...
    'Units', 'normalized', ...
    'Position', [0.020 0.035 0.950 0.245]);
...
buttonTooltips = {'Append a constant-frequency segment.', ...
    'Append a power-ramp segment (order n).', ...
    'Append an exponential-ramp segment (curvature k).', ...
...
moveTooltips = {'Move the selected segment earlier.', ...
    ...
    'Integrate the ODE and plot the robot response with stats.'};
for k = 1:4
...
uicontrol('Parent', infoPanel, 'Style', 'text', ...
    ...
    'HorizontalAlignment', 'left', ...
    'TooltipString', 'Spin-axis moment of inertia (fixed): body + propellers + two magnets. See lecture_notes.md Sec. 3.', ...
    'Units', 'normalized', 'Position', [0.025 0.560 0.420 0.280]);
uicontrol('Parent', infoPanel, 'Style', 'text', ...
    'String', sprintf('k_drag = %.5e N m/Hz^2', k_drag), ...
    'HorizontalAlignment', 'left', ...
    'TooltipString', 'Quadratic drag-torque coefficient from the fit tau_drag = -k*f^2.', ...
    'Units', 'normalized', 'Position', [0.025 0.150 0.420 0.280]);
uicontrol('Parent', infoPanel, 'Style', 'text', ...
    'String', sprintf('Drag-fit R^2 = %.5f', R_squared), ...
    'HorizontalAlignment', 'left', ...
    'TooltipString', 'Goodness of the quadratic drag fit (near 1 validates the f^2 model).', ...
...
frequencyAxes = axes( ...
    'Parent', rightPanel, ...
    'Units', 'normalized', ...
    'Position', [0.085 0.685 0.875 0.245], ...
    'Box', 'on');
...
phaseAxes = axes( ...
    'Parent', rightPanel, ...
    'Units', 'normalized', ...
    'Position', [0.085 0.465 0.875 0.155], ...
    'Box', 'on');
grid(phaseAxes, 'on');
...
verticalAxes = axes( ...
    'Parent', rightPanel, ...
    'Units', 'normalized', ...
    'Position', [0.085 0.245 0.875 0.155], ...
    'Box', 'on');
grid(verticalAxes, 'on');
xlabel(verticalAxes, 'Time (s)');
ylabel(verticalAxes, 'Vertical displacement (mm)');
title(verticalAxes, 'Vertical displacement, upward positive');
...
uicontrol('Parent', rightPanel, 'Style', 'text', ...
    ...
    'HorizontalAlignment', 'left', ...
    'Units', 'normalized', ...
    'Position', [0.035 0.195 0.300 0.030]);
...
resultBox = uicontrol( ...
    ...
    'TooltipString', 'Read-only run summary: tracking errors, torque, phase turns, altitude, per-Hold pass/fail.', ...
    'Units', 'normalized', ...
    'Position', [0.035 0.025 0.925 0.165]);
...
%% ---- GUI callbacks -----------------------------------------------------
    ...
    function resetExample(~,~)
        ...
        set(initialVelocityEdit, 'String', '0');
        set(hoverFrequencyEdit, 'String', '140');
    ...
    function previewCommand(varargin)
        ...
        try
            ...
            xlabel(verticalAxes, 'Time (s)');
            ylabel(verticalAxes, 'Vertical displacement (mm)');
            title(verticalAxes, 'Run Simulation to calculate vertical displacement');
            updateTotalTime();
        catch ME
    ...
    function runSimulation(varargin)
        ...
        initialVerticalVelocity = str2double(get(initialVelocityEdit, 'String'));
        hoverFrequency = str2double(get(hoverFrequencyEdit, 'String'));
        ...
        if ~isfinite(initialVerticalVelocity)
            showError('Initial vertical velocity must be finite.');
            return;
        end
        if ~isfinite(hoverFrequency) || hoverFrequency <= 0
            showError('Lift = weight frequency must be a positive finite number.');
            return;
        end
        ...
        try
            ...
            % Vertical translation model, upward positive. Lift is based on
            % the actual simulated robot rotation frequency, not directly
            % on the external-field command.
            gravity = 9.80665;
            liftToWeight = (fRobot./hoverFrequency).^2;
            verticalAcceleration = gravity.*(liftToWeight - 1);
            verticalVelocity = initialVerticalVelocity + ...
                cumtrapz(allTime, verticalAcceleration);
            verticalDisplacement = cumtrapz(allTime, verticalVelocity);
            ...
            cla(verticalAxes);
            plot(verticalAxes, allTime, 1000*verticalDisplacement, 'LineWidth',1.5);
            ...
            xlabel(verticalAxes, 'Time (s)');
            ylabel(verticalAxes, 'Vertical displacement (mm)');
            title(verticalAxes, 'Vertical displacement from lift proportional to f_{robot}^2');
            ...
            resultLines = {
                ...
                sprintf('Hover frequency           : %.6f Hz', hoverFrequency);
                sprintf('Initial vertical velocity : %.6f m/s', initialVerticalVelocity);
                sprintf('Final vertical velocity   : %.6f m/s', verticalVelocity(end));
                sprintf('Final vertical displacement: %.6f mm', 1000*verticalDisplacement(end));
                sprintf('Minimum vertical displacement: %.6f mm', 1000*min(verticalDisplacement));
                sprintf('Maximum vertical displacement: %.6f mm', 1000*max(verticalDisplacement));
                sprintf('Minimum vertical acceleration: %.6f m/s^2', min(verticalAcceleration));
                sprintf('Maximum vertical acceleration: %.6f m/s^2', max(verticalAcceleration));
