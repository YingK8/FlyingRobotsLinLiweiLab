...
- `solo_sweep.json` — per-channel SOLO frequency sweep: one coil at a time at 70%
  carrier, stepped across 100/120/140/155/170/190/220 Hz and held 6 s per point
  (long enough for a manual gaussmeter read), with a 20 s de-energized gap before
  each channel to reposition the probe. Locates each channel's *own* series-LC
  resonance from its `I_cs` peak — the stand-in for an LCR meter, which cannot be
  replaced by a DC measurement here because the series cap (`docs/PCB_Design_Documentation.md`
  §144, `OUTB → cap → coil return`) blocks DC entirely. Loaded by
  `[env:solo_sweep]` (passthrough). Log sheet:
  `data/2026-07-29_solo_sweep/gauss_log.md`.
