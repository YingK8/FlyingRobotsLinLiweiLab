...
ai/fit_rlc_model.py and the plan's dynamic-model section).
...
with a label like "SWEEP_A_F150" so ai/fit_rlc_model.py can extract the
settled current per (channel, frequency) point.

Usage:
  uv run python ai/gen_solo_sweep_experiment.py --duty 30 \
...
def build_events(
    ...
    for ch in channels:
        ...
        events.append(
            {"method": "activateChannels", "mask": mask, "value": float(duty)}
        )
        for f in freqs:
            events.append({"method": "label", "value": f"SWEEP_{ch}_F{f:g}"})
            events.append(
                {
                    "method": "addLinearRampTask",
                    "from": cur_freq,
                    "to": f,
...
def main() -> None:
    ...
    total_ms = total_duration_ms(
        args.channels, args.freqs, args.dwell_ms, args.transition_ms, args.gap_ms
    )
    print(
        f"{len(args.channels)} channels x {len(args.freqs)} freqs -> "
    ...
    schedule = {
        "resolution_ms": 25,
        "initial_freq": float(args.initial_freq),
