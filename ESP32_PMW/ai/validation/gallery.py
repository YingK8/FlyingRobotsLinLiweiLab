...
def failure_specs():
    ...
    return [
        ...
        (
            34.0,
            210.0,
            np.array([0.0, 0.0, 350.0]),
            dome(0.40),
            0.9,
            0.05,
            14.0,
            "long range",
            "fewer pixels across the rim, so depth degrades",
        ),
    ]
...
def weighting_specs():
    ...
    return [
        (
            ...
            8.0,
            "tilt 70°",
            "the rod pushes the silhouette outward along the short axis",
        ),
        (
            60.0,
            120.0,
...
def build_weighting(width=1024, height=768, gain=50.0):
    ...
    with rendermod.Renderer(width, height) as r:
        ...
        for spec in weighting_specs():
            pair = {
                label: _tile(
                    r, est, spec, gain, legend=(label == "unweighted"), axial=axial
                )
...
def build(width=1024, height=768, gain=50.0):
    ...
    with rendermod.Renderer(width, height) as r:
        ...
        layered = {
            ...
            "fitted vs true ellipse": _tile(
                r, est, spec, gain, layers=("fit", "truth", "text")
            ),
            "orientation vectors": _tile(
                r, est, spec, gain, layers=("fit", "truth", "normal", "text")
            ),
            "both ambiguity branches": _tile(
                r, est, spec, gain, layers=("fit", "normal", "branches", "text")
            ),
            "residual, amplified": _tile(
                r, est, spec, gain, layers=overlaymod.ALL_LAYERS, legend=True
            ),
        }
