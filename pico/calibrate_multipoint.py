...
def main():
    ...
    # ---- report ----
    print(
        f"{'ch':3}{'g A/V':>9}{'K_eff':>8}{'off_fit mV':>11}"
        f"{'off_ramp mV':>12}{'R^2':>8}{'n/drop':>8}{'old A/V':>9}"
    )
    for ch in CH:
        c = cal[ch]
        print(
            f"{ch[-1]:3}{c['g']:9.2f}{R[ch]*c['g']:8.0f}{c['offset_fit']*1000:11.1f}"
    ...
    for ax, ch in zip(axes.ravel(), CH):
        ...
        keep = c["keep"]
        ax.scatter(
            I[keep], V[keep] * 1000, s=20, color=COLORS[ch], zorder=3, label="data"
        )
        if c["n_drop"]:
            ax.scatter(
                I[~keep],
        ...
        ax.plot(
            xx,
            (c["slope"] * xx + c["intercept"]) * 1000,
            "k-",
            lw=1.5,
            label=f"V = {c['offset_fit']*1000:.0f}mV + I/{c['g']:.2f}",
        )
        ax.set_title(
            f"{ch}  g={c['g']:.2f} A/V, K_eff={R[ch]*c['g']:.0f}, " f"R²={c['r2']:.4f}"
        )
    ...
    fig.suptitle(
        "VNH5019 current-sense calibration — multipoint fit " "(supply-current sweep)"
    )
    fig.tight_layout()
