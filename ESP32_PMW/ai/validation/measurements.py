    ...
    try:
        for name, mesh in _mesh_variants(original):
            ...
            for tilt in tilts:
                renderer.mesh = mesh
                s = renderer.render(
                    float(tilt), 0.0, [0.0, 0.0, 220.0], light=light, view=view
                )
                ...
                cells.append(
                    (
                        float(plus.max()) if len(plus) else 0.0,
    ...
    try:
        ...
        for tilt in tilts:
            ...
            if seg is None:
                continue
            truth = conic.normalise_ellipse(
                conic.project_circle(s.center_mm, s.normal, rendermod.RIM_RADIUS_MM, K)
            )
...
def channel_reliability(batch, rig, radius_mm, bands=((20, 40), (40, 55), (55, 75))):
    ...
    for item in batch:
        ...
        for cam, view, seg in zip(rig.cameras, sample.views, pose.per_view):
            ...
            try:
                truth = conic.normalise_ellipse(
                    conic.project_circle(view.center_mm, view.normal, radius_mm, cam.K)
                )
            except Exception:
                continue
...
def calibration_models(dataset=None, bands=((10, 20), (20, 40), (40, 55), (55, 71))):
    ...
    for lo, hi in ((5, 71), (20, 50)):
        try:
            models.append(
                (
                    f"cylinder, fit {lo}–{hi}°",
                    TiltCalibration.fit_cylinder(
                        ratio[tr], tilt[tr], tilt_range=(lo, hi)
                    ),
                )
            )
        except ValueError:
    ...
    for name, cal in models:
        ...
        for lo, hi in bands:
            ...
            cells.append(
                (
                    float(np.median(np.abs(est - tilt[m]))),
        ...
        rows.append(
            {
                ...
                "floor": (
                    None
                    if cal is None or cal.model != "cylinder"
...
def weighting_schemes(batch, rig, radius_mm, bands=((20, 40), (40, 55), (55, 75))):
    ...
    for item in batch:
        ...
        for cam, view, seg in zip(rig.cameras, sample.views, pose.per_view):
            ...
            for name, fn in tune_weighting.SCHEMES.items():
                ...
                acc[name][band].append(cal.tilt(math.degrees(math.acos(r))) - seen)
    ...
    for name in tune_weighting.SCHEMES:
        ...
        for b in bands:
            ...
            cells.append(
                (float(np.median(np.abs(v))), float(np.median(v)))
                if len(v) >= 5
                else None
            )
        rows.append({"name": name, "cells": cells})
...
def major_channel_ab(batch, rig, bands=((20, 40), (40, 55), (55, 75))):
    ...
    for item in batch:
        ...
        if band is None:
            continue
        base = pose.normal
        rows[False][band].append(stereomod.line_angle_deg(base, sample.normal_world))
        ...
        rows[True][band].append(stereomod.line_angle_deg(blended, sample.normal_world))

    out = []
    for label, key in (
        ("ratio channel only (shipped)", False),
...
def wall_model_chart(fit, size=(6.4, 2.8), dpi=150):
    ...
    leg = ax.legend(
        ...
        frameon=True,
        framealpha=1.0,
        edgecolor=scene3d.COL_LINE,
        facecolor=scene3d.PANEL_BG,
    )
