...
def solve_all(d, mask, radius, tilt_cal):
    ...
    for i in idx:
        ellipse = tilt_cal.apply(
            ((d["e_cx"][i], d["e_cy"][i]), (d["major"][i], d["minor"][i]), d["e_deg"][i])
        )
        ...
        norm_best = min(
            poses, key=lambda p: -abs(float(p.normal @ gt_n))
        )

        dxyz.append(pos_best.center - gt_c)
        ang.append(math.degrees(math.acos(min(1.0, abs(float(norm_best.normal @ gt_n))))))
        tilt_err.append(
        ...
        az_err.append(
            (math.degrees(math.atan2(n_up[1], n_up[0]))
             - math.degrees(math.atan2(gt_up[1], gt_up[0])) + 180.0) % 360.0 - 180.0
        )
...
def report(name, r, extra=""):
    ...
    print(f"  {'dof':<16s} {'median':>10s} {'p95':>10s} {'max':>10s}   {'% of range':>22s}")
    for label, v, rel in (
        ...
        print(f"  {label:<16s} {np.median(v):10.3f} {np.percentile(v, 95):10.3f} "
              f"{v.max():10.3f}   {pct(np.median(rel) * 100):>10s} med"
              f" {pct(np.percentile(rel, 95) * 100):>10s} p95")

    print(f"  {'|position|/body':<16s} {np.median(total)/BODY_MM*100:9.2f}% "
    ...
    for label, v in (
        ...
        print(f"  {label:<16s} {np.median(v):10.3f} {np.percentile(v, 95):10.3f} {v.max():10.3f}")
    print(f"  {'tilt bias (deg)':<16s} {r['tilt_err_deg'].mean():+10.3f}  (mean, i.e. the "
          f"systematic part)")
...
def main(argv=None):
    ...
    print("=" * 78)
    print(f"data {Path(args.data).name} | train {train.sum()} | test {test.sum()} "
          f"| {int(d['resolution'][0])}x{int(d['resolution'][1])}")

    base = report("BASELINE (test)", solve_all(d, test, radius0, identity),
    ...
    cal = TiltCalibration.fit(
        theta_raw, tr["tilt"],
        meta={"source": Path(args.data).name, "radius_mm": radius,
              "split": "train", "body_mm": BODY_MM},
    )
    print(f"[fit on train] tilt correction  theta_true = {cal.a:.5f}*t + {cal.b:.6f}*t^2  "
          f"(n={cal.meta['n_samples']})")
    print(f"               maps 10->{cal.tilt(10):.1f}  30->{cal.tilt(30):.1f}  "
          f"50->{cal.tilt(50):.1f}  70->{cal.tilt(70):.1f} deg")

    # --- score on test only ------------------------------------------------
    r_rad = report("+ radius only (test)", solve_all(d, test, radius, identity),
                   f"radius {radius:.4f} mm")
    tuned = solve_all(d, test, radius, cal)
    r_all = report("+ radius + tilt calibration (test)", tuned, f"radius {radius:.4f} mm")
    ...
    for label, before, after in (
        ...
        print(f"  {label:<18s} {before:7.3f} -> {after:7.3f}   "
              f"({(after/before - 1) * 100:+.1f}%)")

    print("\n-- ambiguity, unaffected by any of this (test)")
    m = tuned["margin_deg"]
    print(f"   candidate branches differ by a median of {np.median(m):.1f} deg; one frame")
    ...
    sub = {k: (v[band] if isinstance(v, np.ndarray) and len(v) == len(band) else v)
    ...
    if args.write:
        p = cal.save()
        print(f"\nwrote {p}")
        print(f"NOTE radius: set RADIUS_MM = {radius:.4f} in estimator.py "
              f"(was {radius0:.4f})")
    else:
