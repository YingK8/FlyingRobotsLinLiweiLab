...
levels, so ai/coupling_matrix.py --segments-log can extract a coupling
...
Usage:
    uv run python ai/gen_coupling_experiment.py
    uv run python ai/gen_coupling_experiment.py --levels 50 100 --active-ms 2000
"""
...
def build_events(
    direction: str, levels: list[int], active_ms: int, gap_ms: int
) -> list[dict]:
    ...
    for level in levels:
        for mask, name in SEGMENTS:
            ...
            events.append(
                {"method": "activateChannels", "mask": mask, "value": float(level)}
            )
...
def main() -> None:
    ...
    print(
        f"{len(args.levels)} levels x {len(SEGMENTS)} segments -> "
