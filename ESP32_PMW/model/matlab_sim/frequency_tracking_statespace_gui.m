...
% Settings live on a 16-column x 6-row grid (see gridPos), read as two field
...
heaveTip = ['Heave time constant tau_h = m_R/(k_w*f_hover), s. Sets the inflow air ' ...
    'resistance: z_ddot loses |f_robot|/(f_hover*tau_h)*z_dot, so damping grows with ' ...
    'spin and vanishes at f_robot = 0. Steady climb rate per Hz above hover is ' ...
    '(2*g/f_hover)*tau_h. An assumption pending a hover/climb test -- see ' ...
    'lecture_notes.md Sec. 6.3 and 8.2.'];
...
% Row 5 stays in the two-column split but its right half is a readout: the
% heave pole and climb gain follow directly from the tau_h next to them.
addLabel(settingsPanel, gridPos(1,5,6), 'Heave tau_h (s)', heaveTip);
heaveEdit = addEdit(settingsPanel, gridPos(7,5,2), ...
    num2str(defaults.heaveTimeConstant), heaveTip, @heaveParametersChanged);
heaveLabel = uicontrol('Parent', settingsPanel, 'Style', 'text', ...
    'String', '', 'FontWeight', 'bold', 'FontSize', 10, ...
    'HorizontalAlignment', 'right', ...
    'BackgroundColor', get(settingsPanel, 'BackgroundColor'), ...
    'TooltipString', ['Heave pole -1/tau_h and the steady climb rate per Hz ' ...
        'above hover, (2*g/f_hover)*tau_h.'], ...
    'Units', 'normalized', 'Position', gridPos(9,5,8));

% Row 6 is readouts, not inputs, so it ignores the two-column split: the
...
autoChainCheck = uicontrol('Parent', settingsPanel, 'Style', 'checkbox', ...
    ...
    'Units', 'normalized', 'Position', gridPos(1,6,7), ...
...
driveLabel = uicontrol('Parent', settingsPanel, 'Style', 'text', ...
    ...
    'Units', 'normalized', 'Position', gridPos(8,6,4));
totalTimeLabel = uicontrol('Parent', settingsPanel, 'Style', 'text', ...
    ...
    'Units', 'normalized', 'Position', gridPos(12,6,5));
...
uicontrol('Parent', settingsPanel, 'Style', 'text', ...
    'String', { ...
        ...
        'On a pad until N/W = 1 - (f_robot/f_hover)^2 hits 0; then air resistance -|f|/(f_h*tau_h)*z_dot.'}, ...
    'FontSize', 9, ...
...
%% ---- GUI callbacks -----------------------------------------------------
    ...
    function resetExample(~,~)
        ...
        set(heaveEdit, 'String', num2str(defaults.heaveTimeConstant));
        updateHeaveLabel();
    ...
    function runSimulation(varargin)
        ...
        [heaveTimeConstant, heaveError] = readScalar( ...
            heaveEdit, 'Heave time constant tau_h', true);

        problems = {driveError, tolError, velError, hoverError, heaveError};
        ...
        try
            result = sim.run(data, struct( ...
                ...
                'heaveTimeConstant',       heaveTimeConstant, ...
...
%% ---- Plotting ----------------------------------------------------------
    function plotResult(result)
        ...
        % Altitude on the left, the two non-gravity vertical forces on the
        % right, both as a fraction of weight so they share one scale: the
        % pad reaction N/W, which falls to zero at liftoff and releases z,
        % and the inflow air resistance, which is zero on the pad (z_dot is
        % pinned there), negative while climbing and positive while falling.
        resetVerticalAxes();
        yyaxis(verticalAxes, 'right');
        normalLine = plot(verticalAxes, result.time, result.normalForceRatio, ...
            '-', 'LineWidth',1.2);
        hold(verticalAxes, 'on');
        dragLine = plot(verticalAxes, result.time, result.heaveDragToWeight, ...
            '-', 'LineWidth',1.2);
        hold(verticalAxes, 'off');
        ylabel(verticalAxes, 'force / weight');
        set(verticalAxes, 'YLim', forceLimits( ...
            [result.normalForceRatio; result.heaveDragToWeight]));
        ...
        legend(verticalAxes, [heightLine normalLine dragLine], ...
            {'z (mm)','N / weight (pad)','air resistance / weight'}, ...
...
%% ---- Reading the settings ----------------------------------------------
    ...
    function heaveParametersChanged(~,~)
        updateHeaveLabel();
    end

    function updateHeaveLabel()
        [heaveTimeConstant, heaveError] = readScalar(heaveEdit, 'tau_h', true);
        [hoverFrequency, hoverError] = readScalar( ...
            hoverFrequencyEdit, 'f_hover', true);
        if isempty(heaveError) && isempty(hoverError)
            set(heaveLabel, 'String', sprintf( ...
                'heave pole -%.2f 1/s, climb %.3f (m/s)/Hz', ...
                1/heaveTimeConstant, ...
                2*9.80665*heaveTimeConstant/hoverFrequency), ...
                'ForegroundColor', [0 0 0]);
        else
            set(heaveLabel, 'String', 'heave: invalid', ...
                'ForegroundColor', [0.7 0 0]);
        end
    end
...
%% ---- Layout and formatting helpers -------------------------------------
    function position = gridPos(column, row, span)
        % Cell rectangle on the settings panel's 16-column x 6-row grid.
        ...
        % one of exactly two x positions. Six rows share the same panel the
        % five used to, so the pitch is 0.120 and the cells 0.100 tall; the
        % model-description text below starts at 0.005 and must stay clear
        % of rowBottom(6) = 0.190. That text needs three 9 pt lines, which
        % is ~39 px in the panel's ~244 px, so it gets 0.176 (~43 px) --
        % measure the uicontrol Extent against Position before shrinking it.
        columnWidth = 0.960/16;
        rowBottom = [0.780 0.662 0.544 0.426 0.308 0.190];
