...
while i < len(t):
    if nact[i] == 0:
        ...
        continue
    j = i
    while j < len(t) and nact[j] >= 1 and (active[j] == active[i]).all():
        j += 1
    if (j - i) > 0.5 * fs:  # >0.5s
        mask = active[i]
        segs.append((t[i], t[j - 1], mask))
    i = j
...
for t0, t1, mask in segs:
    ...
    others = " ".join(
        f"{c[-1]}={vals[c]:5.0f}(+{vals[c]-floor[c]:4.0f})"
        for c in chs
        if c not in driven
    )
    print(f"  {t0:5.0f}{t1:6.0f}  {dn:11s} | {others}")
    if len(driven) == 1:
        solo_rows.append((driven[0], vals))
...
out = csv.rsplit(".", 1)[0] + "_analysis.png"
fig.tight_layout()
