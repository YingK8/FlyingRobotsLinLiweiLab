    ...
    for i in range(n):
        ...
        rows.append(
            dict(
                ...
                gt_n=s.normal,
                gt_tilt=tilt,
                raw=np.asarray(pose.xyz_mm),
                raw_n=np.asarray(pose.normal),
                filt=xyz_f,
                filt_n=n_f,
                vel=vel_f,
            )
...
def analyse(rows, dt, fps):
    ...
    print(
        f"  {'channel':<14s} {'raw RMSE':>10s} {'filtered':>10s} {'gain':>8s} {'lag':>9s}"
    )
    ...
    for k, axis in (("x", 0), ("y", 1), ("z", 2)):
        ...
        print(
            f"  {k+' (mm)':<14s} {r_rms:10.3f} {f_rms:10.3f} {r_rms/max(f_rms,1e-9):7.2f}x "
            f"{lag:8.2f}ms"
        )
    ...
    print(
        f"  {'|position|':<14s} {r_pos:10.3f} {f_pos:10.3f} {r_pos/max(f_pos,1e-9):7.2f}x"
    )
    ...
    print(
        f"  {'normal (deg)':<14s} {r_ang:10.3f} {f_ang:10.3f} {r_ang/max(f_ang,1e-9):7.2f}x "
        f"{lag_n:8.2f}ms"
    )
    ...
    for label, v in (
        ...
        e = vrms(v)
        print(
            f"    {label:<30s} {e:8.2f}  ({e/max(speed.mean(),1e-9)*100:5.1f}% of mean speed)"
        )
...
def main(argv=None):
    ...
    ap.add_argument(
        "--fps", type=float, default=None, help="single rate; default sweeps 60/240/420"
    )
    ...
    if len(results) > 1:
        ...
        print("=" * 78)
        print(
            "Position error is correlated frame to frame, so differencing it divides a"
        )
        print("nearly-constant error by an ever smaller dt. The Kalman is unaffected.")
        print(
            f"  {'fps':>6s} {'|pos| raw':>11s} {'filtered':>10s} {'gain':>7s} "
            f"{'v: diff':>9s} {'v: IIR':>8s} {'v: Kalman':>10s}"
        )
        for fps, o in results.items():
            print(
                f"  {fps:6.0f} {o['pos'][0]:11.3f} {o['pos'][1]:10.3f} "
