...
def classify_waveform(thd, harmonic_amps):
    ...
    if (
        0.35 < thd < 0.60
        and 0.25 < r.get(3, 0) < 0.42
        and 0.13 < r.get(5, 0) < 0.27
        and even_energy < 0.10
    ):
        conf = (
            1.0
            - (abs(r.get(3, 0) - 1 / 3) / (1 / 3) + abs(r.get(5, 0) - 1 / 5) / (1 / 5))
            / 2.0
        )
...
def vi_phase(v_samples, i_samples, fs, f0):
    ...
    if abs(phase) < 10.0:
        ...
        regime = (
            "current LEADS -> BELOW resonance (capacitive): raise drive "
            "frequency or decrease series C."
        )
    else:
        regime = (
            "current LAGS -> ABOVE resonance (inductive): lower drive "
...
def configure_channel(chandle, spec):
    ...
    assert_pico_ok(
        ps.ps5000aSetChannel(chandle, ch, int(spec.enabled), coupling, rng, 0.0)
    )
...
def pick_timebase(chandle, target_fs, n_samples):
    ...
    if chosen is None:
        raise SystemExit(
            "Could not find a valid timebase for the requested "
...
def run_block_capture(chandle, config, max_adc):
    ...
    assert_pico_ok(
        ps.ps5000aRunBlock(chandle, pre, post, timebase, None, 0, None, None)
    )
    ...
    return CaptureResult(
        ...
        units=units,
        fs_hz=fs,
        actual_interval_ns=interval_ns,
        overflow=ovf,
        config=config,
    )
...
def make_figure(result, analyses, path, title=None):
    ...
    for ax, label in zip(axes[:n], labels):
        ...
        ax.set_title(
            f"{label}: {a.verdict}  f0={a.f0_hz:.1f} Hz  "
            f"THD={a.thd*100:.1f}%{clip}",
            fontsize=9,
        )
...
def print_summary(result, analyses, vi_pairs):
    ...
    for v_label, i_label in vi_pairs:
        if v_label in result.data and i_label in result.data:
            ...
            phase, regime = vi_phase(
                result.data[v_label], result.data[i_label], result.fs_hz, f0
            )
            print("-" * 70)
            print(
                f"  V-I phase ({i_label} vs {v_label}) @ {f0:.1f} Hz: "
                f"{phase:+.1f} deg"
            )
            print(f"    -> {regime}")
    print("=" * 70)
...
def self_test():
    ...
    for name, x in cases.items():
        ...
        ok &= passed
        print(
            f"  {name:>9}: verdict={a.verdict:<16} f0={a.f0_hz:6.1f} "
    ...
    ok &= phase_ok
    print(
        f"  V-I phase: {phase:+.1f} deg (expected -30) "
...
def main(argv=None):
    ...
    p.add_argument(
        "--preset", choices=["envelope", "carrier", "custom"], default="envelope"
    )
    ...
    if args.list_units:
        ...
        print(
            f"Opened 5000D unit OK. maxADC={max_adc.value}, "
    ...
    if args.dry_run:
        ...
        print(
            f"  target fs : {config.target_sample_rate_hz:.0f} Hz  "
            f"samples: {config.n_samples}"
        )
        for c in config.channels:
            print(
                f"  channel {c.channel}: {c.range_name} {c.coupling} "
        ...
        if _HAVE_PS5000A:
            ...
            print(
                f"  -> timebase {tb}, actual interval {iv:.1f} ns "
    ...
    analyses = {
        l: analyze_channel(
            ...
            result.data[l],
            result.fs_hz,
            clipped=result.overflow.get(l, False),
        )
        for l in result.data
    }

    prefix = (
        args.out_prefix or f"picoscope_{config.preset}_{time.strftime('%Y%m%d_%H%M%S')}"
    )
    if not args.no_csv:
