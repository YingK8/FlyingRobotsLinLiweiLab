...
def get_thread_dataset_tlvs(network_info, network_key):
    ...
    dataset_tlvs = (
        "00030000"
        + int_to_hex_str(channel)
        + "0208"
        + ext_pan_id
        + "0510"
        + network_key
        + "0102"
        + int_to_hex_str(pan_id)
    )
    return dataset_tlvs
...
async def scan_wifi_APs(sel_transport, tp, sec):
    ...
    try:
        message = prov.scan_start_request(
            "wifi", sec, blocking=True, group_channels=group_channels
        )
...
async def scan_thread_networks(sel_transport, tp, sec):
    ...
    try:
        message = prov.scan_start_request(
            "thread", sec, blocking=True, group_channels=0
        )
        start_time = time.time()
        response = await tp.send_data("prov-scan", message)
        stop_time = time.time()
    ...
    try:
        ...
        if await has_capability(obj_transport, "thread_prov"):
            ...
            if args.dataset_tlvs is None:
                if not await has_capability(obj_transport, "thread_scan"):
                    ...
                    while True:
                        ...
                        start_time = time.time()
                        Networks = await scan_thread_networks(
                        ...
                        print(
                            "{0: >4} {1: <8} {2: <18} {3: <18} {4: <18} {5: <4} {6: <4} {7: <16}".format(
                                "S.N.",
                                "PAN ID",
                                "EXT PAN ID",
                                "NAME",
                        ...
                        for i in range(len(Networks)):
                            print(
                                "[{0: >2}] {1: <8} {2: <18} {3: <18} {4: <18} {5: <4} {6: <4} {7: <16}".format(
                                    i + 1,
                                    Networks[i]["pan_id"],
                                    Networks[i]["ext_pan_id"],
                        ...
                        while True:
                            try:
                                select = int(
                                    input("Select Network by number (0 to rescan) : ")
                                )
                        ...
                        if select != 0:
                            ...
                            args.dataset_tlvs = get_thread_dataset_tlvs(
                                Networks[select], network_key
                            )
                            break

            print("\n==== Sending Thread Dataset to Target ====")
            if not await send_thread_config(
                obj_transport, obj_security, args.dataset_tlvs
            ):
            ...
            if not await has_capability(obj_transport, "wifi_prov"):
                print(
                    'Use wifi_provisioning for device without "wifi_prov" capabilities'
                )
