
#pragma once
#include "PwmController.h"
#include "PhaseSequencer.h"
#include <Arduino.h>
#include <FS.h>
#include <SPIFFS.h>
#include <vector>

const uint32_t STEP_SIZE_MS = 25; // Default time step for trajectory points

class CsvPhaseSequencer : public PhaseSequencer {
public:
  CsvPhaseSequencer(PwmController *phaseCtrl);

  /**
   * @brief Load CSV waypoints (SPIFFS) and compile. Header
   *        `time,channel,duty,carrier_duty,frequency,phase`, optional
   *        `# channels,N` / `# step_size_ms,N` / `# interpolation,linear|hermite`
   *        directives above it. Rows sharing a `time` = one full-state point;
   *        points are resampled every resolutionMs and interpolated (directive
   *        picks linear vs. hermite/ease).
   * @return False if the file can't open, has no rows, or its first block
   *         doesn't start at time 0.
   */
  bool loadFromCSVFile(const char *filename,
                       uint32_t resolutionMs = STEP_SIZE_MS);
};
