...
Run: uv run python ai/tests/test_backgrounds.py
"""
...
def test_never_reaches_threshold():
    ...
    print(
        f"  below threshold     max grey per generator: {detail} (limit {bg.THRESHOLD})"
    )
...
def test_contrast_is_meaningful_at_the_edge_tier():
    ...
    print(
        f"  edge is harder      band contrast {np.median(core):.1f} -> "
