...
shipped one is fitted to renders of `flyingrobot_thick _rod2.STL`; if the
...
def calibration_path(name="tilt_calibration", appearance=None):
    ...
    appearance = _segment.APPEARANCE if appearance is None else appearance
...
class TiltCalibration:
    ...
    **What it does not cover.** The mast adds a separate, much sharper
    ...
    def raw_tilt(self, theta_true_deg):
        ...
        if disc < 0:
            return t
        return float(np.clip((-self.a + math.sqrt(disc)) / (2.0 * self.b),
                             0.0, MAX_FIT_TILT_DEG))
    ...
    def fit_cylinder(cls, ratio, theta_true_deg, meta=None, tilt_range=(20.0, 50.0)):
        ...
        keep = (np.isfinite(rho) & np.isfinite(th)
                & (th >= np.radians(tilt_range[0])) & (th <= np.radians(tilt_range[1])))
        rho, th = rho[keep], th[keep]
        if len(rho) < 10:
            raise ValueError(f"need at least 10 samples in tilt {tilt_range}, got {len(rho)}")
        ...
        if not (0.0 <= k < 1.0):
            raise ValueError(f"fitted wall ratio k={k:.4f} is unphysical; expected 0 <= k < 1")

        cal = cls(k=float(k), model="cylinder",
                  meta=dict(meta or {}, n_samples=int(len(rho)),
        ...
        if not np.all(np.diff(raw) > 0):
            raise ValueError("fitted correction is not monotonic above the floor; "
                             "refusing to use it")
        return cal
            ...
            json.dumps(
                {
                    ...
                    "k": self.k,
                    "kind": self.model,
                    "model": ("rho = cos(theta) + k*sin(theta); k = h/R, the duct "
...
class CentreCalibration:
    ...
    **Direction.**  The displacement lies along the projection of the rotor
    axis, and its *sign* follows which way the robot leans.  That sign cannot be
    recovered from the ellipse: an ellipse's angle is defined only modulo 180
    degrees, so the short-axis direction is ambiguous while the lean is not.
    Measured along the raw short axis the offset scatters between -0.0006 and
    +0.0128 of the major axis for the same tilt; measured along the projected
    rotor axis it collapses onto one curve.  So this correction needs the
    **normal**, which means it is applied after a first back-projection rather
    than to the raw ellipse -- see `apply_to_ellipse`.

    **Scale.**  Stored as a fraction of the major axis, which makes it very
    nearly range-independent: measured at 170 mm and 300 mm the fraction agrees
    to about 20%, against a raw pixel offset that changes by 1.5x.

    **What it cannot fix.**  A residual of about +-0.0055 of the major axis
    remains, and it is not noise -- it is the rim's own out-of-roundness (radial
    standard deviation 0.108 mm on a 10.204 mm radius, i.e. 0.0053 of the major
    axis, which matches the measured amplitude closely).  It depends on which
    part of the rim faces the lean, and the robot spins at 310-350 Hz against a
    much slower camera, so that orientation is aliased and unknowable per frame.
    Symmetric lighting does not remove it, which is how it was distinguished
    from a lighting artefact.

    The curve is **not** required to be monotonic, unlike `TiltCalibration`:
    nothing inverts it, it is a lookup.  Which is just as well, because it
    changes sign near 57 degrees as the mast takes over from the wall.
    """
    ...
    def offset(self, tilt_deg):
        ...
        if self.is_identity:
            return 0.0
        return float(np.interp(float(tilt_deg), self.tilt_knots_deg,
                               self.offset_over_major))
    ...
    @classmethod
    def fit(cls, tilt_deg, offset_over_major, knots=(15, 25, 35, 45, 55, 60, 65, 70),
            meta=None):
        ...
        return cls(tilt_knots_deg=tuple(float(k) for k in np.asarray(knots)[ok]),
    ...
    def save(self, path=None):
        ...
        path.write_text(json.dumps({
            ...
            "model": ("centre displacement along the projected rotor axis, as a "
    ...
    def load(cls, path=None):
        ...
        return cls(tilt_knots_deg=tuple(d["tilt_knots_deg"]),
