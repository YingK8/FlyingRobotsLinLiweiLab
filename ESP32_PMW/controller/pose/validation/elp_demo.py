...
FRAMES = HERE.parents[2] / "controller" / "vision" / "drone_orientation" / "elp"
...
def _label(img, text, sub=""):
    ...
    cv2.putText(out, text, (8, 18), cv2.FONT_HERSHEY_SIMPLEX, 0.52, (235, 235, 240), 1,
                cv2.LINE_AA)
    if sub:
        (w, _), _ = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, 0.52, 1)
        cv2.putText(out, sub, (14 + w, 18), cv2.FONT_HERSHEY_SIMPLEX, 0.44,
                    (150, 200, 255), 1, cv2.LINE_AA)
    return out


def _row(panels, scale=0.5):
    panels = [cv2.resize(p, None, fx=scale, fy=scale, interpolation=cv2.INTER_AREA)
              for p in panels]
    h = max(p.shape[0] for p in panels)
    strip = np.full((h, sum(p.shape[1] for p in panels) + PAD * (len(panels) - 1), 3),
                    BG, np.uint8)
    x = 0
    for p in panels:
...
def ungated(gray):
    ...
    """
    return segmod.segment(cv2.bitwise_not(gray), appearance="bright",
                          thresh=segmod.DARK_THRESH)
...
def stages(name, scale=0.5):
    ...
    sil = (cv2.threshold(ch, lvl, 255, cv2.THRESH_BINARY)[1]
           if ch is not None else np.zeros_like(gray))
    ...
    caption = ("no detection" if seg is None else
               f"major {seg.ellipse[1][0]:.0f}px  rms {seg.fit_rms_px:.2f}px  "
...
def comparison(name, scale=0.5):
    ...
    return _row([
        _label(segmod.draw(gray, plain, rejected=False), "ungated threshold",
               cap(plain)),
...
def main(argv=None):
    ...
    print(f"appearance={segmod.APPEARANCE}  level={segmod.DARK_THRESH}  "
    ...
    for name in names:
        ...
        written.append(p)
        print(f"  {name:<18} " +
              ("no detection" if seg is None else
               f"c=({seg.ellipse[0][0]:.0f},{seg.ellipse[0][1]:.0f}) "
        ...
        if gated is not None and plain is not None:
            print(f"  {'':<18} ungated major {plain.ellipse[1][0]:.0f}px vs "
