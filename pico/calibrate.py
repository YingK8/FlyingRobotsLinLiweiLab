...
df = pd.read_csv(ABCD, skiprows=[1])
t = df["Time"].values
...
print(
    f"{'ch':3}{'offset mV':>10}{'gain A/V':>10}{'K_eff':>8}{'lin R^2':>9}{'old A/V':>9}"
)
cal = {}
...
for ax, ch in zip(axes.ravel(), CH):
    ...
    print(
        f"{ch[-1]:3}{offset[ch]*1000:10.1f}{g:10.2f}{Keff:8.0f}{r2:9.3f}{OLD[ch]:9.2f}"
    )
    ...
    ax.set_xlabel("Load current (A, duty-anchored)")
    ax.set_ylabel("CS envelope (V)")
    ax.grid(alpha=0.3)
    ax.legend(fontsize=8, loc="upper left")
fig.suptitle("Two-parameter CS calibration: CS voltage vs current (gain + offset)")
fig.tight_layout()
fig.savefig("calibration_fit.png", dpi=140)
...
fig2.tight_layout()
fig2.savefig("current_ABCD_all_offsetcorr.png", dpi=140)
