...
`CameraSource` is the piece the repo has never had.  Everything camera-side
...
class ImageSource(FrameSource):
    ...
    def __init__(self, frames=None, directory=None, pattern="*.png", rate_hz=None, loop=False):
        if frames is None and directory is None:
            raise ValueError("ImageSource needs either frames or a directory")
...
class CameraSource(FrameSource):
    ...
    def __init__(
        ...
        self._thread = threading.Thread(target=self._grab_loop, name="grabber", daemon=True)
        self._thread.start()
...
class StereoSource(FrameSource):
    ...
    **Sync is the honest weak point of a software-triggered pair.**  Without a
    ...
    def __init__(self, sources, max_skew_s=None, timeout=2.0):
        ...
        if len(self.sources) < 2:
            raise ValueError("StereoSource needs at least two sources")
    ...
    def read(self):
        ...
        for s in self.sources:
            item = s.read() if not isinstance(s, CameraSource) else s.read(self.timeout)
            if item is None:
                return None
            items.append(item)
...
def open_stereo(specs, max_skew_s=None, **kw):
    """Build a `StereoSource` from two CLI-friendly source strings."""
...
def open_source(spec, **kw):
    ...
    if s.startswith("camera"):
        _, _, idx = s.partition(":")
        return CameraSource(index=int(idx) if idx else 0, **kw)

    path = Path(s)
    if path.is_dir():
        return ImageSource(directory=path, **{k: v for k, v in kw.items() if k in
                                              ("pattern", "rate_hz", "loop")})
    if not path.exists():
        raise FileNotFoundError(s)
    return VideoFileSource(path, **{k: v for k, v in kw.items() if k in ("grayscale", "loop")})
