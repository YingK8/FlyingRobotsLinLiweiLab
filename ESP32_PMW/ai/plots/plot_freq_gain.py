...
Usage: uv run python ai/plot_freq_gain.py [CSV]
"""
...
f, tt, Sxx = spectrogram(
    v, fs=fs, nperseg=nperseg, noverlap=noverlap, scaling="spectrum", mode="magnitude"
)
band = f <= FMAX
ax = axs[0]
pcm = ax.pcolormesh(
    tt, f[band], 20 * np.log10(Sxx[band] + 1e-9), shading="gouraud", cmap="magma"
)
...
for c in chs:
    ...
    gb = np.array(
        [gsel[bi == k].mean() if np.any(bi == k) else np.nan for k in range(nb)]
    )
...
ax.set_ylabel("current-sense amplitude (mV)")
ax.set_xlim(0, 220)
ax.grid(alpha=0.3)
