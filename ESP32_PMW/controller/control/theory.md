...
## 24. The alignment-rate experiment: ramp, labels and heat

Cut two of the four coils while the rotor is spinning and the robot leans to a new
equilibrium. How fast it leans, as a function of drive frequency, is the alignment rate.
The measurement is `controller/control/alignment_rate.py`; the schedule is generated by
`tilt_schedule.py` and driven by `tilt_run.py`. This section records what had to be fixed to
make it run at all.

### 24.1 One ramp segment cannot both capture and accelerate

The refined sweep schedule (merge-base `01f3354`) used a single EASE segment of
`24000 + 100 f` ms, fitted over 20-160 Hz targets. Extrapolated to a 10 Hz target that gives
25 s to cover a span of 8 Hz: 0.32 Hz/s average, 0.64 Hz/s peak. Run on the rig 2026-09-10,
the rotor tracked it, but the whole ramp crawled — the rule is almost all base term, and the
base was sized for spans an order of magnitude larger.

The tension is real and not a tuning error. Capture needs *time below* the ~4.9 Hz pull-in
crossing (18.3), and the dwell **fraction** of an EASE ramp rises as the target falls — 43%
of the ramp at a 10 Hz target against 11% at 210 Hz. A single segment at a low target is
therefore almost entirely inside the capture band by construction, and any duration that
gives it enough dwell also makes the rest glacial.

Two segments separate the two jobs:

- **2 -> 5 Hz over 5 s.** Exists to be slow. Contributes 4.2 s below the crossing, at every
  target, by construction rather than by fit.
- **5 Hz -> target at 3.5 Hz/s.** Exists to accelerate.

Total ramp becomes 6.4 s at 10 Hz and 40.7 s at 130 Hz, against 25 s and 37 s before. The
rate is not the binding constraint anywhere: 18.3 measures $\dot f_{max}$ at 77-127 Hz/s
through the 6-10 Hz band, so 3.5 Hz/s carries better than 10x margin. The segments must abut
exactly — a gap between them is a commanded frequency step, which `ramp.check` refuses on the
`seq=` path for the same reason.

### 24.2 A label placed last labels nothing

`JsonPwmSequencer` treats `label` as state, not as a task: *"Label active for whatever step
gets pushed next; label entries update this without pushing a queue entry of their own"*
(`JsonPwmSequencer.cpp:216`). A `label` at the end of a schedule therefore tags no step, never
becomes `stepLabel()`, and the firmware prints a bare `label=`.

Measured 2026-09-10: two 10 Hz takes executed the entire schedule correctly — `FREQ`,
`SETTLE`, `HOLD`, `KILL`, `DOWN` all printed, the ramp and the drop both visible in the
telemetry — and both were recorded as `timeout`, because `tilt_sweep.run()` ends on exactly
`label=TILT_OFF` and got `label=` instead. The take is otherwise perfect; only the terminator
was missing.

The closing label must precede a real step. `tilt_schedule` follows `TILT_OFF` with a 500 ms
wait, and its self-check now asserts that every label is followed by at least one
queue-pushing entry.

This also explains why every earlier `tilt_sweep` run on this branch ended by its 560 s
timeout: the `tilt.json` shipped here has no `TILT_OFF` label at all.

### 24.3 Heat is not proportional to energised seconds

`coil_thermal` charges a flat 0.5 C/s for every energised second and knows nothing about
frequency. But dissipation goes as $I^2$, and coil current is strongly frequency-dependent
below resonance. From 2406 telemetry samples across 13 existing `sweep.log`s, commutated
sum $|I|$:

| f | 20 | 40 | 60 | 80 | 100 | 150-190 |
|---|---|---|---|---|---|---|
| sum I | 2.37 A | 4.61 | 6.68 | 10.41 | 13.82 | 13.94 |
| implied rate | 0.014 C/s | 0.055 | 0.115 | 0.279 | 0.491 | 0.5 |

The flat rate is right only at the top, which is where it was calibrated — and that plateau
is also where the bench supply's 100 W (10 V x 10 A) ceiling binds, which is why the two
agree. A 10 Hz point costs about 3% of what a 100 Hz point costs.

`tilt_run.heat_c` integrates $I^2\,dt$ from the telemetry the firmware already prints and
scales it to that anchor. One correction was necessary: **gate on commanded duty, not on
measured current.** Coils A and C read −1.28 and −1.04 A with every carrier at 0% — the known
sense fault — so summing $|I|$ unfiltered bills a parked board, and it charged ~1 C to a
10 Hz run that drew almost nothing. Zero duty is zero drive whatever the ADC says. A residual
offset remains *during* drive, which over-charges the low-frequency points; that is the safe
direction and it is not yet corrected.

None of this is a thermometer. There is no temperature sensor on this rig, and on 2026-09-10
the flat book had drifted to 98 C — inflated by a run of 95 s timeouts that drew nearly
nothing — while demanding 11 minutes of cooling for a point the $I^2$ integral measures at
0.46 C. Gating on a number that wrong is delay, not safety. The operator's instruction was to
run without the gate; `tilt_run --no-cool` does that, still recording the measured heat per
repeat so the record exists even when nothing acts on it. What remains is the 10 A supply
limit and the GPIO14 button.

### 24.4 The I^2 model was right, and running without the gate is what cooked the coils

The campaign of 2026-09-10 ran seven chunks — 10, 20, 40, 50, 60, 70, 80 Hz, five repeats
each — with `tilt_run --no-cool`, at the operator's instruction. `heat_c` booked them as:

| chunk | 10 | 20 | 40 | 50 | 60 | 70 | 80 | total |
|---|---|---|---|---|---|---|---|---|
| C added | 1.2 | 2.1 | 7.5 | 10.7 | 16.2 | 26.1 | 42.1 | **105.9** |

The operator then measured the coils at **100 C**. From 22 C ambient, with whatever cooling
happened in the gaps, that is close agreement: **the $I^2$ integration predicted the outcome
it got.** It is a model of a rig with no temperature sensor, fitted from telemetry current
against a 0.5 C/s anchor at the ~14 A plateau, and it tracked a 78 C excursion.

So the lesson is not about the model. It is that the model was consulted and then overridden
every single run. Two things made that easy to do:

1. **The flat book was inflated at the moment it was first dismissed.** Before the schedule
   was fixed (24.2), a run of 95 s timeouts drew almost no current while `link.close()`
   charged each one 0.5 C/s of wall time, so `coil_thermal` reached 98 C on a rig that was
   nearly cold. Dismissing *that* number was correct. Continuing to dismiss it once real
   drive started was not, and the two situations look identical from the log.
2. **The cost is superlinear and the cheap chunks come first.** 10 and 20 Hz together cost
   3.3 C, which makes the gate look absurd. 80 Hz alone costs 42.1 C — more than the six
   chunks before it combined. By the time the gate mattered it had already been disabled,
   and nothing re-raised the question.

The remaining chunks are worse still: 90, 100, 120 and 130 Hz are each about 55, 62, 65 and
66 C for five repeats. **Every one of them exceeds the 48 C headroom on its own**, so no
scheduling of them back-to-back is safe even from ambient, and they have to be run either
with the gate on or at two to three repeats instead of five.

Corollary for the runner: `--no-cool` should not be a flag that stays set for a campaign. The
per-chunk heat is known before the chunk starts (`plan()` prints it), and a chunk whose
predicted cost exceeds the remaining headroom is exactly the case the gate exists for.

### 24.5 Above ~50 Hz the repeats stop being one population

The five repeats at 20 and 40 Hz are indistinguishable: a clean step to +7.1 and +7.6 deg with
a repeat-to-repeat sample SD of 0.28 and 0.63 deg. A single flight at those frequencies is
good to under a degree.

From 50 Hz that stops being true, and not gradually:

| f | median | MAD | mean | sample var | sample SD | outliers |
|---|---|---|---|---|---|---|
| 20 | 7.08 | 0.17 | 7.20 | 0.08 | 0.28 | 0 / 5 |
| 40 | 7.57 | 0.76 | 7.42 | 0.39 | 0.63 | 0 / 5 |
| 50 | 4.38 | 1.90 | 7.66 | 51.8 | 7.20 | 1 / 5 |
| 60 | 7.94 | 0.95 | 13.67 | 517 | 22.7 | 2 / 5 |
| 70 | 17.03 | 6.90 | 25.39 | 414 | 20.3 | 1 / 4 |

At 50 Hz four repeats sit at +2 to +8 deg and one settles at +21. At 60 Hz three sit at +8,
one at **+52**, and one at **-8**. The median barely moves; the mean and the variance explode.

**These are not estimator failures.** Three things say so. The outlier traces are smooth,
well-formed curves with the same ringing as their neighbours -- a broken pose solution gives
spikes and discontinuities, not a clean settle at a different level. The ellipse conditioning
is flat across the whole range (axis ratio 0.58-0.67, orientation scatter 3.2-4.6 deg, area
cv 0.11-0.19 at every frequency from 20 to 70 Hz), so nothing about the measurement degrades
where the variance appears. And the two artefacts that DID exist were found and removed first:
sign flips, which read as ~180-theta and put 161 deg frame-to-frame jumps into the vector
angle while the LINE angle moved only 29.9 (fixed by measuring every angle sign-invariantly,
`alignment_rate._hemisphere`), and a suspected cone-branch ambiguity, which a temporal prior
on the branch choice did not change at all -- so it was not that either.

What is left is the robot. It occasionally settles into a different equilibrium, and the onset
is right at the ~52 Hz where 17 and 18.13 put the rotor's saturation on rod friction.

Two consequences. **Report the median and the MAD above 40 Hz**, because a mean over a mixture
describes neither population; `campaign()` prints both and counts repeats further than 3 MAD
out. And **five repeats is the minimum, not a luxury**: at 60 Hz a single flight has a 2-in-5
chance of recording behaviour that is not the modal response, and nothing in that flight's own
data would reveal it.

### 24.6 The failures are visible BEFORE the cut

24.5 recorded that above ~50 Hz the repeats stop being one population, and left it there:
some runs settle somewhere else, the median survives, the mean does not. The operator's
description of what that looks like on the bench -- *"the drone tilts up and fails"* --
turned out to be measurable, and to happen earlier than the event being measured.

Pre-cut radial tilt, taken over the second before the coils are cut, referenced to the
coils-off rest attitude:

| population | n | median | IQR | range |
|---|---|---|---|---|
| modal repeats | 36 | 16.78 deg | 13.9-17.9 | 9.6-19.8 |
| outlier repeats | 7 | 39.13 deg | — | 4.4, 8.8, 19.2, **39.1, 40.5, 43.0, 45.2** |

Four of the seven outliers were already sitting at **39-45 deg** when the cut happened,
against a normal 16-19. After the cut they fall back to a normal 18-20 deg. So these are not
runs whose RESPONSE was anomalous; they are runs whose INITIAL CONDITION was wrong, and the
transient was then measured from the wrong place. The tilt-up happens during the ramp or the
level hold, before anything is asked of the robot.

A band of median +- 3 robust sigma over the campaign's own pre-cut tilts flags **6 of the 7
outliers and none of the 36 good runs**. `alignment_rate.PRE_TILT_SIGMA`.

Three reasons this is better than the modal-basin test it supplements:

* **It is causal.** It asks whether the run started where it was supposed to, not whether the
  answer came out near the others -- so it cannot reject a real effect for being unusual.
* **It is per-take.** The modal test needs the population, so a single flight cannot be
  judged; the precondition can, which matters when one flight is all there is.
* **It could run live.** The pre-cut window closes before the cut, so a host watching the
  pose could refuse to fire the drop at all rather than spending a point on a bad start.
  That is the version worth building if this experiment is repeated.

The one outlier it misses read 19.2 deg pre-cut -- inside the band, and still ended up at
14.2 deg of azimuth against a modal 48.6. So a good start is necessary and not sufficient,
and the modal-basin count stays as the second filter.

**Not established:** why the robot tilts up on some runs and not others at the same commanded
frequency. It is not the drive -- the aborted 100 Hz attempts drew 13.68 and 12.68 C, normal
for that frequency -- and it is not the pose pipeline, whose ellipse conditioning is flat
across the whole range (20.5). Rod friction saturating near 52 Hz (17, 18.13) is the standing
suspect for the 50-70 Hz band, but it does not explain 100 Hz needing three attempts on a
night when 80 and 90 Hz ran 5/5.

### 24.7 The rate is only measurable where the rise outlasts the smoother

The angle and the rate are not equally well determined, and the reason is the filter rather
than the robot.

The tilt trace carries a once-per-rev cone that has to be removed before any timing can be
read, and it is removed with a boxcar of a whole number of rotor revolutions -- an exact null
at the spin frequency and its harmonics (`rev_window`). That window is about 0.2-0.25 s at
every frequency, because `round(0.25 f) / f` is 0.25 s by construction.

A rise faster than that window cannot be measured through it. At 10 Hz the five repeats give:

| repeat | amplitude | 10-90% rise | rate |
|---|---|---|---|
| 1 | 41.89 deg | 0.20 s | 168 deg/s |
| 2 | 45.92 | 0.58 | 63 |
| 3 | 40.48 | **0.09** | 360 |
| 4 | 43.32 | 0.46 | 75 |
| 5 | 39.94 | **1.22** | 26 |

The AMPLITUDES agree to a few degrees -- 39.9 to 45.9, five of five in the modal basin. The
RISE TIMES span 0.09 to 1.22 s, a factor of thirteen, and repeat 3's entire rise is shorter
than the smoothing window, so its `t10`/`t90` describe the filter and not the motion.

So at 10 Hz the rate is not a measurement. The same caution applies to any point whose rise
approaches 0.2 s.

**An earlier version of this section said the ANGLE at 10 Hz was sound and only the rate was
not. Ten repeats withdrew that.** On the first five the angle read 42.3 +- 2.4 deg, CV 0.06 --
as tight as anywhere in the set. On ten it reads 37.7 +- 9.0, CV 0.24, four times the scatter
of 20-40 Hz, and three of the ten rises fall under the smoothing window. The first five were
unrepresentatively clean.

That is the general lesson from taking the campaign to ten points, and it cut both ways:
50 Hz went 3/5 -> 8/10 (better than it looked), 60 Hz went 3/5 -> 5/9 (genuinely unreliable,
not bad luck), and 10 Hz went from the tightest angle in the set to the loosest. **Five
repeats misled in both directions**, which is worth remembering before quoting any five-repeat
number from this rig.

10 Hz being weakest on both axes fits the physics rather than contradicting it: it draws
~1.2 A against 13.8 A at 100 Hz, so it has the least authority over the robot and friction
dominates both where it ends up and how fast it gets there.

Part of the spread is real: 10 Hz draws ~1.2 A against 13.8 A at 100 Hz, so the restoring
torque is weakest there and the timing is most exposed to friction. But the two causes cannot
be separated with this filter, which is the honest statement.

**Two corrections that had to be made to the rate before it meant anything**, both found by
the operator reading the plot:

* **A rate cannot be negative.** `rate = 0.8 * d_azimuth / (t90 - t10)` inherits the sign of
  the step, and failed runs whose azimuth went the other way (-68 deg at 50 Hz) were entering
  the statistics as -200 deg/s. An alignment rate is a speed; the direction already lives in
  the azimuth. Magnitudes only.
* **Failed runs are not slow alignments.** Restricting the rate statistics to the modal basin
  collapsed 70 Hz's variance from 28862 to 23.5 and 60 Hz's from 7757 to 421. Including them
  had been measuring the spread between two populations and calling it measurement error.

With both applied the rate rises cleanly with drive -- about 121 deg/s at 20 Hz to 213 at
80 Hz -- which is the frequency dependence the angle does NOT have (24.5). The drive sets how
fast the robot gets there; the coil geometry sets where it goes.

### 24.8 GPIO14 already restarts, on the second press

`reset_button.h` prints two different lines and they are different events: press 1 is
`forceAllGatesLow()` plus `"-- gates off, press again to restart"` and a park; press 2 is
`"-- restarting"` and `ESP.restart()`, which re-runs the schedule from step 0 with no
persisted state anywhere. A substring test for `[block]` cannot tell them apart, which is
what `tilt_sweep` did.

The firmware is deliberately left alone. For a `main_tilt` run exactly two things
de-energise the coils — the GPIO14 press and `tilt_sweep.park()` — because the viser `stop`
cannot reach a firmware that parses no serial, and `safe_off.py` is worse than useless here:
opening its port resets the board and restarts the schedule. Making one press reboot would
leave `park()` alone, over a hardcoded port, from a host that must still be alive. So press 1
stays an unconditional hard stop, and the host does the restarting.

An aborted repeat is discarded rather than analysed. A restarted run sharing a take would
give its `sweep.log` two `FREQ_<f>HZ` labels, and `alignment_rate.timeline()` builds one point
per `FREQ_` label — the analysis would silently gain a duplicate point whose first half is a
truncated ramp. `tilt_sweep.stills()` refuses incomplete points for the same reason.

### 24.9 What 10 Hz says

First result, five repeats: **no response.** Step amplitude −0.02 deg against a 2 deg floor,
so no rate is reported at all rather than one fitted to noise — the failure `pose/body_angle`
records, where `max|d(tilt)/dt|` returned 250-1000 deg/s at every frequency including the
ones with no response, is precisely what that floor exists to prevent. The operator's
independent observation at the bench was the same: the rotor tracked the field and never
leaned.

That is consistent with 24.3 rather than surprising. The lean comes from the field asymmetry
that cutting A and C creates, and at 10 Hz the coils carry ~1.2 A against 13.8 A at 100 Hz.
There should be a frequency floor below which this experiment has no signal, and locating it
is worth the 0.06 C a 10 Hz point costs.
