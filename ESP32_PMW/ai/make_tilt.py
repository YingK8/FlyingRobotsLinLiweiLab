...
ap.add_argument(
    ...
    type=int,
    default=None,
    help="只覆盖第 2 段（5 Hz→f）的时长；缺省按 tilt_schedule 的两段捕获斜坡",
)
ap.add_argument(
    "--seg2-rate",
    type=float,
    default=None,
    help="第 2 段速率 Hz/s；缺省 2.8（≥60 Hz）/ 3.5，同 tilt_schedule",
)
ap.add_argument(
    "--hold-ms",
    type=int,
    default=None,
    help="缺省：≤100 Hz 用 5000，更高用 3000（2026-09-16 起；之前一律 5000）",
)
...
ap.add_argument(
    ...
    type=int,
    default=14000,
    help="全部线圈归零后的等待时长；与 reset-ms 合计为两次之间的停机时间（缺省 1+14=15 s；whole 时合计 ×1.5=22.5 s）",
)
ap.add_argument(
    "--reset-ms",
    type=int,
    default=1000,
    help="断电状态下频率 f→2 Hz 的时长，排在 off 之前",
)
ap.add_argument("--out", default="spiffs_data/tilt.json")
...
if a.seg2_rate is not None:
    _ts.SEG2_RATE_OVERRIDE = a.seg2_rate
up = _ts.ramp_tasks(f)
if a.ramp_ms is not None and len(up) > 1:
    up[-1] = dict(up[-1], duration_ms=int(a.ramp_ms))
ramp = sum(t["duration_ms"] for t in up)
hold = a.hold_ms if a.hold_ms is not None else (5000 if f <= 100 else 3000)
off = a.off_ms
if a.design == "whole":
    off = (
        round((a.reset_ms + a.off_ms) * 1.5) - a.reset_ms
    )  # 总停机 ×1.5；降频段仍 1 s，加长的全在全关等待里
sch = []
for k in range(1, a.n + 1):
    sch += [
        {"method": "label", "value": f"RUN_{k}_SPINUP_{f:g}HZ"},
        ...
        *up,
        ...
        {
            "method": "addEaseRampTask",
            "from": f,
            "to": _ts.RAMP_FROM_HZ,
            "duration_ms": a.reset_ms,
        },
...
period = (ramp + hold + a.post_ms + a.reset_ms + off) / 1000
drive = (ramp + hold + a.post_ms) / 1000
print(
    f"# [{a.design}] {f:g} Hz × {a.n} 次，每次 {period:.1f} s（斜坡 {ramp/1000:g} + 保持 {hold/1000:g} + 断 A+C {a.post_ms/1000:g} + 降频 {a.reset_ms/1000:g} + 全关 {off/1000:g}）",
    file=sys.stderr,
)
print(
    "# 斜坡两段："
    + " + ".join(f"{t['from']:g}→{t['to']:g} {t['duration_ms']/1000:g}s" for t in up),
    file=sys.stderr,
)
print(
    "# 断 A+C 的时刻（EN 之后）："
    + ", ".join(f"{(ramp+hold)/1000+(k-1)*period:.0f} s" for k in range(1, a.n + 1))
    + f"；全部结束 {a.n*period:.0f} s",
    file=sys.stderr,
)
print(
    f"# 每次之间线圈全部为 0 共 {(a.reset_ms+off)/1000:g} s（其中降频 {a.reset_ms/1000:g} s 也不通电）",
    file=sys.stderr,
)
