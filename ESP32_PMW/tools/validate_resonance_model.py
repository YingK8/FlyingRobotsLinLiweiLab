...
Compares tools/rlc_fit.json's per-channel f0_i = 1/(2*pi*sqrt(L*C)) (and, if
tools/m_matrix.json is given, the coupling-severity peak
kappa_ij(omega) = omega*M_ij / |Z_i(omega)|, expected to also peak near f0_i
-- see the plan's dynamic-model section) against
tools/pid_metrics.py's resonance_peak_freq_hz from a REAL
...
Usage:
  uv run python tools/validate_resonance_model.py current_pid_run.log \
      --rlc-fit tools/rlc_fit.json [--m-matrix tools/m_matrix.json] --tol-hz 15
"""
...
def main() -> None:
    ...
    ap.add_argument("--rlc-fit", required=True, help="tools/rlc_fit.json")
    ap.add_argument("--m-matrix", default=None,
                     help="tools/m_matrix.json -- if given, also checks the full "
