...
EspSecCertSigECDSA = Struct(
    ...
    "public_key"
    / Bytes(
        lambda ctx: 49 if ctx.algorithm == 1 else (65 if ctx.algorithm == 2 else 97)
    ),  # curve_id(1) + key_size
    "signature"
    / Bytes(
        lambda ctx: 48 if ctx.algorithm == 1 else (64 if ctx.algorithm == 2 else 96)
    ),  # Signature size
)
...
def _calculate_padding(data_length: int) -> int:
    ...
    return (
        MIN_ALIGNMENT_REQUIRED - (data_length % MIN_ALIGNMENT_REQUIRED)
...
class TlvPartitionBuilder:
    ...
    def add_certificate(
        self, tlv_type: tlv_type_t, cert_path: Union[str, bytes], subtype: int = 0
    ) -> None:
        """
    ...
    def add_private_key(
        ...
        self.add_tlv_entry(
            tlv_type_t.ESP_SECURE_CERT_PRIV_KEY_TLV, subtype, key_bytes, flags
        )

    def add_ds_data(
        self, ciphertext: bytes, iv: bytes, rsa_key_len: int, subtype: int = 0
    ) -> None:
        """
    ...
    def add_ds_context(
        self, efuse_key_id: int, rsa_key_len: int, subtype: int = 0
    ) -> None:
        """
        ...
        ds_context = Struct(
            ...
            "rsa_key_len"
            / Int16ul,  # 2 bytes RSA key length (16-bit unsigned little-endian)
        ).build(
            {
                "reserved": 0,
        ...
        self.add_tlv_entry(
            tlv_type_t.ESP_SECURE_CERT_DS_CONTEXT_TLV, subtype, ds_context, 0
        )
    ...
    def add_signature_block(
        ...
        length = (
            partition_length if partition_length is not None else self.current_offset
        )
        ...
        self.add_tlv_entry(
            tlv_type_t.ESP_SECURE_CERT_SIGNATURE_BLOCK_TLV, subtype, sig_block_data, 0
        )

    def add_tlv_entry(
        self, tlv_type: Union[int, tlv_type_t], subtype: int, data: bytes, flags: int
    ) -> None:
        """
        ...
        if self.current_offset + len(complete_tlv) > PARTITION_SIZE:
            raise ValueError(
                f"Partition size exceeded: {self.current_offset + len(complete_tlv)} > {PARTITION_SIZE}"
            )

        self.partition_data[
            self.current_offset : self.current_offset + len(complete_tlv)
        ] = complete_tlv
        ...
        print(
            f"Added TLV entry: type={tlv_type}, subtype={subtype}, length={len(complete_tlv)}"
        )
    ...
    def build_partition(self, output_file: str) -> None:
        ...
        print(
            f"Total partition size used: {self.current_offset} / {PARTITION_SIZE} bytes"
        )
...
class EspSecureCert:
    ...
    def __init__(self, version: str = "2", hash_type: str = "sha256"):
        ...
        self.bin_filename = os.path.join(
            esp_secure_cert_data_dir, "esp_secure_cert.bin"
        )
        self.signed_bin_filename = os.path.join(
            esp_secure_cert_data_dir, "esp_secure_cert_signed.bin"
        )
    ...
    def _is_certificate_entry(self, tlv_type: Union[int, tlv_type_t]) -> bool:
        ...
        return tlv_type in [
            tlv_type_t.ESP_SECURE_CERT_CA_CERT_TLV.value,
            tlv_type_t.ESP_SECURE_CERT_DEV_CERT_TLV.value,
        ]
    ...
    def _validate_entry(self, entry: dict) -> bool:
        ...
        if (
            self._is_certificate_entry(entry.get("tlv_type"))
            or self._is_private_key_entry(entry.get("tlv_type"))
        ) and entry.get("data_type") != "file":
            entry["data_value"] = self._parse_data_from_any_format(
                entry["data_value"], entry["data_type"], True
            )
    ...
    def _parse_data_from_any_format(self, data_value, data_type, output_file=False):
        ...
        if output_file:
            temp_file = os.path.join(
                esp_secure_cert_data_dir,
                f"temp_key_{hash(str(data_value)) % 10000}.key",
            )
    ...
    def _check_for_duplicate_tlv_entries(self, entry):
        ...
        for existing_entry in self.secure_cert_entries:
            if existing_entry.get("tlv_type") == entry.get(
                "tlv_type"
            ) and existing_entry.get("tlv_subtype") == entry.get("tlv_subtype"):
                print(
                    f"WARNING: Duplicate entry found for type {entry.get('tlv_type')}, subtype {entry.get('tlv_subtype')}"
                )
            ...
            for line_num, row in enumerate(reader, 1):
                ...
                try:
                    ...
                    entry = {
                        ...
                        "efuse_key_file": (
                            row["efuse_key"] if row["efuse_key"] else None
                        ),
                        "ds_enabled": row["priv_key_type"].lower()
                        in ["rsa_ds", "ecdsa_peripheral"],
                    }
    ...
    def add_entry(self, entry):
        ...
        if entry["tlv_type"] == tlv_format.tlv_type_t.ESP_SECURE_CERT_PRIV_KEY_TLV:
            ...
            entry["ds_enabled"] = entry["priv_key_type"] in (
                "rsa_ds",
                "ecdsa_peripheral",
            )
        ...
        try:
            ...
            for tlv_type, type_entries in entries_by_type.items():
                ...
                for entry in type_entries:
                    print(
                        f"  - Subtype {entry['tlv_subtype']}: {entry['data_type']} format"
                    )
            ...
            if ds_enabled_entries:
                ...
                if port:
                    ...
                    print(
                        "WARNING: DS keys will be generated locally but NOT burned to device"
                    )
                    print(
                        "WARNING: You will need to manually burn the keys to device eFuse later"
                    )
                ...
                for entry in ds_enabled_entries:
                    ...
                    if entry["algorithm"] == "RSA":
                        ...
                        if efuse_key_file:
                            print(
                                f"  - Using provided eFuse key file: {efuse_key_file}"
                            )
                        else:
                            print(
                                f"  - Will generate new HMAC key file: {hmac_key_file}"
                            )
            ...
            for ds_tlv_entry in ds_tlv_entries:
                if ds_tlv_entry["algorithm"] == "RSA":
                    self.builder.add_ds_data(
                        ds_tlv_entry["c"],
                        ds_tlv_entry["iv"],
                        ds_tlv_entry["rsa_key_len"],
                        ds_tlv_entry["subtype"],
                    )
                    self.builder.add_ds_context(
                        ds_tlv_entry["efuse_id"],
                        ds_tlv_entry["rsa_key_len"],
                        ds_tlv_entry["subtype"],
                    )
                elif ds_tlv_entry["algorithm"] == "ECDSA":
                    self.builder.add_security_config(
                        ds_tlv_entry["efuse_id"], ds_tlv_entry["subtype"]
                    )
            ...
            for entry in self.secure_cert_entries:
                ...
                priv_key_type = (
                    entry["priv_key_type"] if "priv_key_type" in entry else None
                )
                configure_ds_enabled = (
                    entry["ds_enabled"] if "ds_enabled" in entry else None
                )

                print(
                    f"Processing: Type {tlv_type}, Subtype {tlv_subtype}, Data Type: {data_type}"
                )

                # Process data based on type
                try:
                    processed_data = self._parse_data_from_any_format(
                        data_value, data_type, False
                    )
                    ...
                    if self._is_certificate_entry(tlv_type):  # CA_CERT or DEV_CERT
                        cert_type_name = (
                            ...
                            else "Device Certificate"
                        )
                        print(f"  Adding {cert_type_name} (subtype {tlv_subtype})")
                        self.builder.add_certificate(
                            tlv_type_t(tlv_type), processed_data, tlv_subtype
                        )
                        processed_count += 1
                    # Handle private key
                        ...
                        if priv_key_type == "plaintext":
                            print(
                                f"  Adding private key as plaintext for subtype {tlv_subtype}"
                            )
                            self.builder.add_private_key(
                                processed_data,
                                None,
                                tlv_format.tlv_priv_key_type_t.ESP_SECURE_CERT_DEFAULT_FORMAT_KEY,
                                tlv_subtype,
                            )
                            processed_count += 1
                        elif priv_key_type == "rsa_ds" and configure_ds_enabled:
                            print(
                                f"  Skipping private key TLV for subtype {tlv_subtype} (using hardware RSA DS)"
                            )
                            processed_count += 1
                            pass  # RSA DS private key is handled by DS data and context
                        elif (
                            priv_key_type == "ecdsa_peripheral" and configure_ds_enabled
                        ):
                            print(
                                f"  Adding private key for hardware ECDSA DS for subtype {tlv_subtype}"
                            )
                            self.builder.add_private_key(
                                processed_data,
                                None,
                                tlv_format.tlv_priv_key_type_t.ESP_SECURE_CERT_ECDSA_PERIPHERAL_KEY,
                                tlv_subtype,
                            )
                            processed_count += 1
                        ...
                        if data_type == "file":
                            ...
                            self.builder.add_tlv_entry(
                                tlv_type, tlv_subtype, processed_data, 0
                            )
                            processed_count += 1
                            ...
                            if (
                                isinstance(processed_data, str)
                                and processed_data.lower().endswith(".bin")
                                and os.path.isfile(processed_data)
                            ):
                                with open(processed_data, "rb") as bin_f:
                                    bin_data = bin_f.read()
                                self.builder.add_tlv_entry(
                                    tlv_type, tlv_subtype, bin_data, 0
                                )
                                print(
                                    f"  Added binary data from {processed_data} (subtype {tlv_subtype})"
                                )
                            else:
                                self.builder.add_tlv_entry(
                                    tlv_type, tlv_subtype, processed_data, 0
                                ...
                                print(
                                    f"  Skipping unknown TLV type {tlv_type} (subtype {tlv_subtype})"
                                )
                            processed_count += 1
            ...
            if ds_enabled_entries:
                ...
                if not port:
                    print(
                        "IMPORTANT: DS keys were generated locally. You must burn them to device eFuse manually:"
                    )
                    for entry in ds_enabled_entries:
                        efuse_key_file = get_efuse_key_file(entry["efuse_key_file"])
                        key_file = (
                        ...
                        if entry["algorithm"] == "RSA":
                            print(
                                f"  espefuse.py --chip {target_chip} -p <PORT> burn_key BLOCK_KEY{entry['efuse_id']} {key_file} HMAC_DOWN_DIGITAL_SIGNATURE"
                            )
                        elif entry["algorithm"] == "ECDSA":
                            print(
                                f"  espefuse.py --chip {target_chip} -p <PORT> burn_key BLOCK_KEY{entry['efuse_id']} <ecdsa_key.pem> ECDSA_KEY"
                            )
    ...
    def flash_esp_secure_cert_partition(
        self, idf_target, port, sec_cert_part_offset, flash_filename
    ):
        """
        ...
        if not port:
            ...
            print(
                "INFO: Partition has been generated successfully but not flashed to device"
            )
            print(
                f"To flash manually: esptool.py --chip {idf_target} -p <PORT> write_flash {sec_cert_part_offset} {flash_filename}"
            )
            return

        flash_command = (
            f"esptool.py --chip {idf_target} "
            + f"-p {port} write_flash "
            + f" {sec_cert_part_offset} {flash_filename}"
        )
        try:
            flash_command_output = subprocess.check_output(flash_command, shell=True)
    ...
    def parse_esp_secure_cert_bin(bin_file_path):
        ...
        if not os.path.isfile(bin_file_path):
            print(
                f"ERROR: The provided file '{bin_file_path}' does not exist or is not a file."
            )
            sys.exit(-1)
        ...
        try:
            ...
            if len(data) < 4:
                print(
                    f"ERROR: The provided .bin file '{bin_file_path}' is too small to be a valid TLV partition."
                )
                sys.exit(-1)
            ...
            if magic != TLV_MAGIC:
                print(
                    f"ERROR: The provided .bin file '{bin_file_path}' does not have a valid TLV_MAGIC header (found 0x{magic:08X})."
                )
                sys.exit(-1)
        except Exception as e:
            print(
                f"ERROR: Failed to read or validate the .bin file '{bin_file_path}': {e}"
            )
            sys.exit(-1)
        ...
        if not tlv_entries or len(tlv_entries) == 0:
            print(
                f"ERROR: No valid TLV entries found in the provided .bin file '{bin_file_path}'."
            )
            print(
                "This file may not be a valid ESP Secure Cert partition or may be corrupted."
            )
            sys.exit(-1)
        ...
        for entry in tlv_entries:
            ...
            # DS_DATA (if not already handled)
                ...
                if not any(
                    r["tlv_type"] == "ESP_SECURE_CERT_DS_DATA_TLV"
                    and r["tlv_subtype"] == s
                    for r in csv_rows
                ):
            ...
            # DS_CONTEXT (if not already handled)
                ...
                if not any(
                    r["tlv_type"] == "ESP_SECURE_CERT_DS_CONTEXT_TLV"
                    and r["tlv_subtype"] == s
                    for r in csv_rows
                ):
            ...
            # # SEC_CFG (if not already handled)
                ...
                if not any(
                    r["tlv_type"] == "ESP_SECURE_CERT_SEC_CFG_TLV"
                    and r["tlv_subtype"] == s
                    for r in csv_rows
                ):
        ...
        if not csv_rows or len(csv_rows) == 0:
            print(
                f"ERROR: No valid TLV entries could be processed from the .bin file '{bin_file_path}'."
            )
        ...
        for row in csv_rows:
            ...
            if tlv_length > 0:
                print(
                    f"        TLV type: {tlv_type}, subtype: {tlv_subtype}, length: {tlv_length}"
                )
            else:
                print(f"        TLV type: {tlv_type}, subtype: {tlv_subtype}")
        print(
            f"  - tlv_entries_raw.txt: {os.path.join(output_dir, 'tlv_entries_raw.txt')}"
        )
    ...
    def _parse_tlv_entries_from_bin(bin_file_path):
        ...
        while offset < len(data):
            try:
                ...
                tlv_data = data[
                    tlv_data_offset : tlv_data_offset + tlv_header["length"]
                ]
                entry = {"header": tlv_header, "data": tlv_data, "offset": offset}
                tlv_entries.append(entry)
                total_tlv_size = (
                    header_size + tlv_header["length"] + padding_length + 4
                )  # +4 for CRC footer
    ...
    def _extract_public_key_and_algorithm(
    ...
    def _sign_data_with_espsecure(
        self, private_key_path: str, bin_data: bytes, algorithm: int
    ) -> bytes:
        """
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(
                mode="rb", suffix="_signed.bin", delete=False
            ) as temp_output:
            ...
            try:
                ...
                if magic != ECDSA_SIG_BLOCK_MAGIC:
                    raise ValueError(
                        f"Invalid signature block magic byte: 0x{magic:02X}, expected 0x{ECDSA_SIG_BLOCK_MAGIC:02X}"
                    )
                ...
                if algorithm == ECDSA192_ALGORITHM:
                    ...
                    signature = sig_block[
                        101 : 101 + 48
                    ]  # 48 bytes: 24 bytes r + 24 bytes s
    ...
    def add_signature_block_using_existing_key(
        self,
        data_file_path: str,
        signing_key_file: Union[List[str], str],
        signing_scheme: str,
    ):
        """
        ...
        try:
            for keyfile_path in signing_key_file:
                ...
                algorithm, public_key = self._extract_public_key_and_algorithm(
                    keyfile_path
                )
                ...
                if algorithm == RSA3072_ALGORITHM:
                    with open(keyfile_path, "rb") as keyfile:
                        signature_block = generate_signature_block_using_private_key(
                            keyfiles=[keyfile], contents=bin_data
                        )
                    signature = signature_block
                else:  # ECDSA192, ECDSA256, ECDSA384
                    signature = self._sign_data_with_espsecure(
                        keyfile_path, bin_data, algorithm
                    )

                self.builder.add_signature_block(
                    algorithm,
                    public_key,
                    signature,
                    self.signature_block_no,
                    original_partition_length,
                )
