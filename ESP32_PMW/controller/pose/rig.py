...
DEFAULT_INTRINSICS = Path(__file__).resolve().parents[1] / "vision" / "camera_intrinsics.npz"
...
class StereoRig:
    ...
    def monocular(cls, K=None, dist=None, intrinsics_path=DEFAULT_INTRINSICS):
        ...
        if K is None:
            ...
            if dist is None:
                dist = np.asarray(d["dist_coeffs"], dtype=np.float64).ravel()
        return cls(cameras=(Camera(K=K, dist=dist, name="A"),), meta={"source": "monocular"})
    ...
    def axis_separation_deg(self, i=0, j=1):
        ...
        """
        d = float(np.clip(abs(self.cameras[i].optical_axis @ self.cameras[j].optical_axis), 0, 1))
        return math.degrees(math.acos(d))

    def baseline_mm(self, i=0, j=1):
        return float(np.linalg.norm(self.cameras[i].position - self.cameras[j].position))
    ...
    def predicted_sigma_mm(self, sigma_lat_mm, sigma_depth_mm):
        ...
        return np.sqrt(np.linalg.eigvalsh(self.position_covariance(sigma_lat_mm, sigma_depth_mm)))

    def summary(self):
        """One-line-per-fact description, for provenance headers and logs."""
        out = {
            "n_cameras": len(self.cameras),
            "axis_separation_deg": round(self.axis_separation_deg(), 3) if len(self) > 1 else 0.0,
    ...
    def load(cls, path=DEFAULT_PATH):
        ...
        cams = tuple(
            Camera(
                ...
                dist=None if c.get("dist") is None else np.array(c["dist"], dtype=np.float64),
...
def main(argv=None):
    """Print the geometry of a rig -- handy for choosing angles before rendering."""
    import argparse

    ap = argparse.ArgumentParser(description=__doc__.splitlines()[0])
    ap.add_argument("--rig", help="load a rig JSON instead of building one")
    ap.add_argument("--elev", type=float, nargs=2, default=(45.0, 45.0),
                    help="elevation above horizontal, degrees, per camera")
    ap.add_argument("--azim", type=float, nargs=2, default=(0.0, 90.0),
                    help="bearing about world +z, degrees, per camera")
    ap.add_argument("--range-mm", type=float, default=DEFAULT_RANGE_MM)
    ap.add_argument("--scale", type=float, default=1.0,
                    help="intrinsics scale, e.g. 0.488 for 500 px from a 1024 px calibration")
    ap.add_argument("--sigma-lat", type=float, default=0.078,
                    help="per-view lateral sigma, mm (README held-out value at 1024x768)")
    ap.add_argument("--sigma-depth", type=float, default=0.857,
                    help="per-view depth sigma, mm")
    ap.add_argument("--out", help="write the rig to this path")
    args = ap.parse_args(argv)

    rig = StereoRig.load(args.rig) if args.rig else StereoRig.from_spherical(
        elev_deg=args.elev, azim_deg=args.azim, range_mm=args.range_mm
    )
    if args.scale != 1.0:
        rig = rig.scaled(args.scale)

    for k, v in rig.summary().items():
        print(f"{k:24s} {v}")

    # Per-view sigmas scale inversely with pixel density.
    s = 1.0 / args.scale
    sig = rig.predicted_sigma_mm(args.sigma_lat * s, args.sigma_depth * s)
    print(f"{'per-view sigma_lat_mm':24s} {args.sigma_lat * s:.4f}")
    print(f"{'per-view sigma_depth_mm':24s} {args.sigma_depth * s:.4f}")
    print(f"{'fused sigma per axis':24s} {np.array2string(sig, precision=4)}")
    print(f"{'fused worst axis':24s} {sig.max():.4f} mm")

    if args.out:
        print(f"wrote {rig.save(args.out)}")
    return 0
