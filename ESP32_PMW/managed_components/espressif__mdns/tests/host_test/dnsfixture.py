...
class DnsPythonWrapper:
    ...
    def send_and_receive_query(self, query, timeout=3):
        ...
        try:
            ...
            with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as sock:
                ...
                response_data, _ = sock.recvfrom(
                    512
                )  # 512 bytes is the typical size for a DNS response
    ...
    def run_query(self, name, query_type="PTR", timeout=3):
        ...
        query = dns.message.make_query(
            name, dns.rdatatype.from_text(query_type), dns.rdataclass.IN
        )
    ...
    def check_record(self, name, query_type, expected=True, expect=None):
        ...
        if expected:
            assert any(
                expect in answer for answer in answers
            ), f"Expected record '{expect}' not in answer section"
        else:
            assert not any(
                expect in answer for answer in answers
            ), f"Unexpected record '{expect}' found in answer section"
    ...
    def check_additional(
        ...
        if expected:
            assert (
                found
            ), f"Expected {rdtype_text} for {owner_contains} in Additional not found"
        else:
            assert (
                not found
            ), f"Unexpected {rdtype_text} for {owner_contains} found in Additional"
