...
def split3(samples, seed=0):
    ...
    return (pick(fit_m), pick(sel_m), pick(rep_m),
            sorted(fit_m), sorted(sel_m), sorted(rep_m))


def choose_threshold(samples, target_pos, target_ang, blunder_pos=5.0,
                     blunder_ang=10.0, quantiles=None, verbose=True):
    ...
    *Where on the plateau.*  Coverage as a function of the quantile is a step:
    ...
    for q in quantiles:
        pooled = []
        for held in modes:
            fit = [s for s in samples if s["mode"] != held
                   and s["pos_err"] <= blunder_pos and s["ang_err"] <= blunder_ang]
            if len(fit) < 40:
                continue
            m = uncertainty.ErrorModel.fit(
                [r["features"] for r in fit], [r["pos_err"] for r in fit],
                [r["ang_err"] for r in fit], quantile=float(q))
            pooled += [s for s in samples if s["mode"] == held
                       and s["tier"] == "core" and m.accepts(s["features"],
                                                             target_pos, target_ang)]
        if not pooled:
            continue
        cov = np.mean([s["pos_err"] <= target_pos and s["ang_err"] <= target_ang
                       for s in pooled])
        ...
        if verbose:
            print(f"    q={q:.2f}  accept {len(pooled)/core_n:6.1%}  "
...
def evaluate(model, samples, target_pos, target_ang, label=""):
    ...
    print(f"  {label:<12} accepted {acc:6.1%} of {len(samples):5d}   "
          f"coverage {cov:7.2%}   worst accepted: {worst_pos:.3f} mm / "
          f"{worst_ang:.3f}°")
    return {"accepted": acc, "coverage": cov, "n": len(samples),
            "worst_pos": worst_pos, "worst_ang": worst_ang}
...
def main(argv=None):
    ...
    cache = Path(args.samples) if args.samples else (
        RESULTS / f"error_samples_{args.poses}_{args.seed}.json")
    if cache.exists():
    ...
    print(f"  select pool:  {', '.join(sorted(set(fm) | set(sm)))}  "
    ...
    chosen = choose_threshold(select_pool, uncertainty.TARGET_POS_MM,
                              uncertainty.TARGET_ANGLE_DEG,
                              args.blunder_pos_mm, args.blunder_ang_deg,
    ...
    fit_rows = [s for s in select_pool
                if s["pos_err"] <= args.blunder_pos_mm
                and s["ang_err"] <= args.blunder_ang_deg]
    print(f"  fitting scale on {len(fit_rows)}/{len(select_pool)} "
          f"(excluded {len(select_pool) - len(fit_rows)} blunders)")
    model = uncertainty.ErrorModel.fit(
        [r["features"] for r in fit_rows], [r["pos_err"] for r in fit_rows],
        [r["ang_err"] for r in fit_rows], quantile=q,
        meta={"source": "resolution_sweep", "poses": args.poses, "seed": args.seed,
              "chosen_quantile": q, "plateau": list(plateau),
              "select_modes": sorted(set(fm) | set(sm)), "report_modes": rm})

    print("\nfitted scale coefficients")
    for name, coef in (("position", zip(uncertainty.FEATURES_POS, model.coef_pos)),
    ...
    ev = {"chosen_quantile": q, "plateau": list(plateau),
          "select_acceptance": sel_acc}
    ev["report"] = evaluate(model, rep_s, uncertainty.TARGET_POS_MM,
                            uncertainty.TARGET_ANGLE_DEG, "report all")
    for tier in ("core", "edge"):
        sub = [s for s in rep_s if s["tier"] == tier]
        if sub:
            ev[f"report_{tier}"] = evaluate(model, sub, uncertainty.TARGET_POS_MM,
                                            uncertainty.TARGET_ANGLE_DEG,
                                            f"report {tier}")

    out = RESULTS / "error_model_fit.json"
    out.write_text(json.dumps({"evaluation": ev, "quantile": args.quantile,
                               "poses": args.poses, "seed": args.seed}, indent=2))
    print(f"\nwrote {out}")
    if args.write:
