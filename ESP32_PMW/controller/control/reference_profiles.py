...
class Segment:
    ...
    def __post_init__(self):
        if self.type not in SEGMENT_TYPES:
            raise ValueError(
                f"unknown segment type {self.type!r} (use {SEGMENT_TYPES})"
            )
        if self.duration <= 0:
...
class Profile:
    ...
    def hold(cls, x: float = 0.0, z: float = 0.0, duration: float = 1e9) -> "Profile":
        return cls(
            AxisProfile([Segment("hold", x, x, duration)]),
    ...
    def from_json(cls, path: str) -> "Profile":
        with open(path) as f:
            d = json.load(f)
        return cls(
            AxisProfile([Segment(**s) for s in d["lateral"]]),
            AxisProfile([Segment(**s) for s in d["vertical"]]),
        )

    def to_json(self, path: str) -> None:
        d = {
            "lateral": [asdict(s) for s in self.lateral.segments],
