...
def lighting(rng, n):
    ...
    for i in range(n):
        ...
        if rng.random() < 0.5:
            rigs.append(rendermod.LightRig(dome=((rng.uniform(30, 80), rng.uniform(0, 360)),),
                                           ambient=amb, intensity=rng.uniform(6, 20)))
        else:
            rigs.append(rendermod.LightRig(lateral_deg=(rng.uniform(0, 360),),
                                           ambient=amb, intensity=rng.uniform(6, 20)))
    return rigs


def build(n_train, n_test, width, height, seed=20260806, realistic=True, subframes=7,
          appearance="bright"):
    ...
    rows = {k: [] for k in (
        "tilt_deg", "az_deg", "cx_mm", "cy_mm", "cz_mm",
        "nx", "ny", "nz", "e_cx", "e_cy", "major", "minor", "e_deg",
        "area_px", "fit_rms_px", "alpha", "bg", "ambient", "seg_ms", "split", "iou",
        "exposure_s", "sigma", "spin_hz",
    )}
    ...
    with rendermod.Renderer(width, height) as r:
        K = r.K
        for i in range(n):
            s = r.render(float(tilt[i]), float(az[i]), centres[i],
                         alpha=float(alphas[i]), light=rigs[i], bg_level=float(bgs[i]),
                         exposure=exps[i], body_colour=body)
            ...
            for k, v in (
                ("tilt_deg", s.tilt_deg), ("az_deg", s.azimuth_deg),
                ("cx_mm", s.center_mm[0]), ("cy_mm", s.center_mm[1]), ("cz_mm", s.center_mm[2]),
                ("nx", s.normal[0]), ("ny", s.normal[1]), ("nz", s.normal[2]),
                ("e_cx", ecx), ("e_cy", ecy), ("major", major), ("minor", minor), ("e_deg", edeg),
                ("area_px", seg.area_px), ("fit_rms_px", seg.fit_rms_px),
                ("alpha", alphas[i]), ("bg", bgs[i]), ("ambient", rigs[i].ambient),
                ("seg_ms", seg.t_ms), ("split", 0 if i < n_train else 1),
            ...
            if (i + 1) % 200 == 0:
                ...
                print(f"  {i+1}/{n}  {el:5.1f}s elapsed, {el/(i+1)*(n-i-1):5.1f}s left", flush=True)
...
def main(argv=None):
    ...
    data = build(args.train, args.test, args.width, args.height,
                 realistic=not args.clean, subframes=args.subframes,
    ...
    print(f"wrote {out}  ({n_tr} train, {n_te} test, "
