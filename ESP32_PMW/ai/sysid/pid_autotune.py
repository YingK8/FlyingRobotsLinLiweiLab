...
def run_trial(port, kp, ki, kd, ramp, run_seconds, out_path):
    ...
    try:
        ...
        with open(out_path, "w") as f:
            ...
            if not saw_boot:
                print(
                    f"  WARNING: no boot banner within {BOOT_BANNER_TIMEOUT_S}s "
            ...
            if not gains_applied:
                print(
                    "  WARNING: no ARMED banner seen -- gains may not have "
        ...
        try:
            ...
            print(
                f"  WARNING: failed to send e-stop ({e}) -- verify coils "
                "are off by hand"
            )
        comm.close()
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        default=os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "autotune_runs"
        ),
    )
    args = ap.parse_args()
    ...
    if args.gains_file:
        ...
        print(
            f"seeding from {args.gains_file}: kp0={args.kp0:.3f} "
    ...
    with open(results_path, "w") as rf:
        rf.write(
            "trial,kp,ki,kd,ramp,gains_applied,hold_spread,ss_err,settle_s,resonance_peak,score\n"
        )
        ...
        def try_gains(candidate: dict) -> float:
            ...
            print(
                f"[{n}] kp={candidate['kp']:.3f} ki={candidate['ki']:.3f} "
            ...
            rf.write(
                f"{n},{candidate['kp']},{candidate['ki']},{candidate['kd']},"
            ...
            rf.flush()
            print(
                f"  {format_metrics_line(metrics)} -> score={s:.4f}"
