...
OUTPUT_3D_MATRIX_NAME = (
    "image_to_3d_matrix_custom"  # generated output 3D matrix in the .c and .h file
)
OUTPUT_BMP_ARRAY_NAME = (
    "image_to_bmp_array_custom"  # generated output BMP array in the .c and .h file
)
...
if not PIL_INSTALLED:
    print(
        "Installing Pillow package by running command -m pip install --upgrade Pillow"
    )
    try:
...
class Image3DMatrix:
    ...
    def check_pixels(height: str, width: str):
        ...
        if not height.isdecimal() and width.isdecimal():
            print(
                f"Provided image target width {width} and/or height {height} not valid"
            )
            sys.exit()
    ...
    def check_if_pixels_in_range(height: int, width: int):
        ...
        if (
            height > ESP_IOT_AZURE_DISPLAY_HEIGHT + 1
            or width > ESP_IOT_AZURE_DISPLAY_WIDTH + 1
            or height < 1
            or width < 1
        ):
            print(
                f"Provided image target width {width} and/or height {height} \
        ...
        if height % 2 or width % 2:
            print(
                f"Provided image target width {width} and height {height} must be even"
            )
            sys.exit()
    ...
    def create_paths(self):
        ...
        python_dir = os.path.dirname(
            abspath
        )  # get the absolute path of the folder, in which the python script is located
        ...
        self.result_dir = os.path.join(
            os.path.dirname(os.getcwd()), "3d_matrix", "3d_matrix_data"
        )
    ...
    def get_termianl_input(self):
        ...
        if (len(self.terminal_args) == 2) and (str(self.terminal_args[1]) == "--help"):
            ...
            print(
                f"Too few arguments run following command for help\npython {PYTHON_FILE_NAME} --help"
            )
            sys.exit()
...
def fromat_comas_spaces(
    element_index: int, array_len: int, elements_per_line: int, c_file
):
    """
...
def ouptput_files_gen(img3d: Image3DMatrix, bmp_arr: list, matrix_arr: list):
    ...
    with open(
        os.path.join(img3d.templates_dir, f"{TEMPLATE_FILE_NAME}.c"),
        "r",
        encoding="utf8",
    ) as c_template_file:
        with open(
            os.path.join(img3d.result_dir, output_c_file_name), "w+", encoding="utf8"
        ) as c_file:

            c_file.write(
                f"// File generated by {PYTHON_FILE_NAME}\n"
            )  # write first line to c_file
            c_file.write(
                f"// Image file converted to 3D matrix {img3d.img_name}.{img3d.img_ext.lower()}\n"
            )
            ...
            for line in c_template_file:
                ...
                if "const" in line:
                    ...
                    if img3d.img_include_bmp:
                        c_file.write(
                            f"const uint8_t {OUTPUT_BMP_ARRAY_NAME}[{len(bmp_arr)}] = {{\n\n    "
                        )
                    ...
                    c_file.write(
                        f"const float {OUTPUT_3D_MATRIX_NAME}[{len(matrix_arr)}][4] = {{\n\n    "
                    )
    ...
    with open(
        os.path.join(img3d.templates_dir, f"{TEMPLATE_FILE_NAME}.h"),
        "r",
        encoding="utf8",
    ) as h_template_file:
        with open(
            os.path.join(img3d.result_dir, output_h_file_name), "w+", encoding="utf8"
        ) as h_file:
            ...
            for line in h_template_file:
                ...
                if "extern const" in line:
                    ...
                    if img3d.img_include_bmp:
                        h_file.write(
                            f"extern const uint8_t {OUTPUT_BMP_ARRAY_NAME}[{len(bmp_arr)}];\n"
                        )

                    # include 3d matrix header
                    h_file.write(
                        f"extern const float {OUTPUT_3D_MATRIX_NAME}[{len(matrix_arr)}][4];\n"
                    )
                    next(h_template_file)
                    continue
...
def create_matrix(
    img3d: Image3DMatrix, pixels: list, bmp_img_array: list, matrix_img_array: list
):
    """
...
def convert_img(img3d: Image3DMatrix):
    ...
    img = img.resize(
        (int(img3d.img_width), int(img3d.img_height)), resample=Image.NEAREST
    )
    ...
    if (res_width, res_height) != (img3d.img_width, img3d.img_height):
        ...
        converted_img_location = os.path.join(
            img3d.converted_img_dir, converted_img_name
        )
        img.save(
            converted_img_location, format=img3d.img_ext, subsampling=0, quality=100
        )
        query = "\n".join(
            [
