...
def run_server(server_stop, port, server_ip, client_ip, auth, auth_user, auth_password):
    ...
    try:
        ...
        if auth:
            ...
            subprocess.run(
                f"echo '{auth_user} * {auth_password} *' | sudo tee -a /etc/ppp/chap-secrets",
                shell=True,
            )
        else:
        ...
        while not server_stop.is_set():
            if p.poll() is not None:
                if p.poll() == 16:
                    ...
                    raise ValueError(
                        "ENV_TEST_FAILURE: PPP terminated unexpectedly with {}".format(
                            p.poll()
                        )
                    )
...
def test_examples_protocol_pppos_connect(dut):
    ...
    if server_port is None:
        print("ENV_TEST_FAILURE: Cannot locate PPPD port")
        raise
    ...
    try:
        ...
        while "ppp0" not in netifaces.interfaces():
            print(
                "PPP server haven't yet setup its netif, list of active netifs:{}".format(
