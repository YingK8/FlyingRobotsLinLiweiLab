...
def kappa_peak_freq(
    rlc_all: dict, m_matrix: dict, channel: str, freqs_hz: np.ndarray
) -> float:
    """
...
def main() -> None:
    ...
    ap.add_argument(
        "log", help="main_current_pid.cpp serial log (a real HOLD/RAMP_UP run)"
    )
    ...
    if measured is None:
        raise SystemExit(
            f"{args.log}: no RAMP_UP samples in the 90-190Hz resonance "
    ...
    print(
        f"measured resonance_peak_freq_hz = {measured:.1f} Hz "
