...
class TestCaseInsights(unittest.TestCase):
    ...
    def login(self):
        ...
        response = requests.post(
            url=uri, data=body, headers=header, verify=False, cookies=None
        )
    ...
    def test_100_get_crash_count(self):
        ...
        crash_count = 0
        filt = (
            '[{"f":"Node.ID","o":"keyword","v":["%s"]},{"f":"Type","o":"keyword","v":["crash"]}]'
            % node_id
        )
        ...
        while (curr_ts - from_ts) < max_time:
            url = (
                self.diag_uri
                + "/query/filters/suggest?from_ts={}&to_ts={}&filters={}&fieldname=Type".format(
                    from_ts, curr_ts, encoded_filter
                )
            )
            # Act ----------------------------------------------------------------------------
            resp_crash_filter = requests.get(
                url=url, verify=False, headers=header, cookies=None
            )
            json_resp_crash_filter = json.loads(resp_crash_filter.text)["list"]
            # Assert -------------------------------------------------------------------------
            self.assertEqual(
                resp_crash_filter.status_code,
                200,
                "S100.1 Unexpected status code for {} \n".format(node_id),
            )
            # Check if any crash is present
            if len(json.loads(resp_crash_filter.text)["list"]):
                self.assertEqual(
                    json_resp_crash_filter[0]["key"],
                    "crash",
                    "S100.2 key is not present for {}".format(node_id),
                )
                self.assertLessEqual(
                    json_resp_crash_filter[0]["count"],
                    5,
                    "S100.3 Crash count is not same as expected for {}".format(node_id),
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--username", default="", type=str, help="username for login ")
    parser.add_argument("--password", default="", type=str, help="password for login")
    parser.add_argument(
        "--base_uri",
        default="",
        type=str,
