...
def test_axis_directions(r):
    ...
    print(
        f"  axis directions     +x right, +y down, +z away  (base centre {base[0]:.0f},{base[1]:.0f})"
    )
...
def test_matches_analytic_projection(r):
    ...
    print(
        f"  vs analytic proj    worst centre {worst_c:.2f} px, worst axis {worst_a:.2f} px"
    )
...
def test_ground_truth_normal_consistent(r):
    ...
    print(
        f"  tilt -> axis ratio  worst deviation from cos(tilt) {worst:.4f} (to {MAX_MODEL_TILT_DEG:g} deg)"
    )
...
def test_high_tilt_model_limit(r):
    ...
    print(
        "  high-tilt limit     tilt |  major fit/true  |  minor fit/true  | minor excess"
    )
    worst_major = 0.0
...
def test_alpha_and_background(r):
    ...
    peaks = [
        r.render(0.0, 0.0, [0.0, 0.0, 200.0], alpha=a).image.max() for a in (0.3, 1.0)
    ]
    assert peaks[1] > peaks[0], f"alpha had no effect on brightness: {peaks}"

    bgs = [
        int(
            np.median(r.render(0.0, 0.0, [0.0, 0.0, 200.0], bg_level=b).image[:40, :40])
        )
        for b in (0.0, 0.3)
    ]
    ...
    print(
        f"  alpha / background  peak {peaks[0]}->{peaks[1]}, bg {bgs[0]}->{bgs[1]}, mask ok"
    )
...
def test_estimator_on_render(r):
    ...
    print(
        f"  estimator on render worst pos {worst_pos:.2f} mm, normal {worst_ang:.2f} deg "
...
def test_ambiguity_is_irreducible(r):
    ...
    for tilt in (5.0, 20.0, 40.0):
        ...
        chosen = math.degrees(
            math.acos(float(np.clip(abs(p.normal @ s.normal), -1.0, 1.0)))
        )
...
def test_face_on_conditioning(r):
    ...
    assert (
        max(pos_errs) < 2.0
    ), f"position degraded near face-on: {max(pos_errs):.2f} mm"
    assert axis_errs[0] > axis_errs[1], "face-on axis error should be the worst"
    assert (
        max(axis_errs[1:]) < 6.0
    ), f"off-axis tilt should be well conditioned: {axis_errs}"
