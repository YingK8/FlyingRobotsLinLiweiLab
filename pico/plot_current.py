...
def load(csv):
    return pd.read_csv(csv, skiprows=[1])  # row 1 is units
...
def lbl(ch):
    return f"{ch}" + ("" if TRUST[ch] else " (low-conf)")
...
ax.set_title(
    "Calibrated current — linear carrier ramp 0->100% @ 190 Hz (separate grounds)"
)
ax.set_xlim(t.min(), t.max())
ax.grid(alpha=0.3)
ax.legend(loc="upper left")
fig1.tight_layout()
fig1.savefig("current_ABCD_all.png", dpi=140)
...
fig2.tight_layout()
fig2.savefig("current_ABCD_grid.png", dpi=140)
...
ax.set_xlim(ta.min(), ta.max())
ax.grid(alpha=0.3)
ax.legend(loc="upper left")
fig3.tight_layout()
fig3.savefig("current_A_only.png", dpi=140)

print("peak calibrated current (A):")
for ch in CHANNELS:
    print(
        f"  {ch}: {np.nanmax(env(df[ch].values*SENS[ch])):.2f} A   ({'trust' if TRUST[ch] else 'low-conf'})"
    )
