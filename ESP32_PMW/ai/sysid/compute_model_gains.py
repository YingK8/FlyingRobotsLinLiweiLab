#!/usr/bin/env python3
"""
IMC-PID gain computation from the fitted RLC model (ai/rlc_fit.json),
...
Usage:
  uv run python ai/compute_model_gains.py ai/rlc_fit.json \
      --v-supply 11.9 --channel A --out ai/model_gains.json
"""
...
def continuous_to_firmware_gains(
    Kc: float, Ti: float, Td: float, nominal_tick_ms: float
):
...
def main() -> None:
    ...
    ap.add_argument("rlc_fit", help="ai/rlc_fit.json from ai/fit_rlc_model.py")
    ap.add_argument(
        "--v-supply",
        type=float,
        required=True,
    ...
    ap.add_argument(
        "--out", default=os.path.join(os.path.dirname(__file__), "model_gains.json")
    )
    args = ap.parse_args()
    ...
    worst = max(
        table, key=lambda r: r["K_a_per_pct"]
    )  # highest gain = at/near resonance
    print(
        f"recommended seed (worst-case @ {worst['freq_hz']:.0f} Hz, "
