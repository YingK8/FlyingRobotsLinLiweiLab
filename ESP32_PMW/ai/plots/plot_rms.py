...
print(
    f"{args.csv}: {len(df)} samp, {t[-1]:.1f}s, {fs:.0f} S/s, "
    f"RMS window {win} samp ({args.win_ms:.0f} ms)"
)
print(f"{'ch':<10}{'overall RMS(mV)':>16}{'peak RMS(mV)':>14}")
for c in chs:
    print(
        f"{c:<10}{np.sqrt(np.mean((df[c].values-df[c].values.mean())**2))*1000:>16.3f}"
...
ax.set_title(
    f"Rolling-RMS current-sense envelope — {args.csv}  "
...
ax = axs[1]
ax.plot(t, rms["Channel A"] + rms["Channel B"], lw=1.3, color="#8c564b", label="A + B")
ax.plot(t, rms["Channel C"] + rms["Channel D"], lw=1.3, color="#17becf", label="C + D")
ax.set_title("Board-pair RMS sum — closer curves = better balanced supplies")
ax.set_xlabel("time (s)")
ax.set_ylabel("summed RMS (mV)")
ax.grid(alpha=0.3)
