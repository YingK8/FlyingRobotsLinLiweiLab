...
Run: uv run python ai/tests/test_calibration.py
...
from shape import MAX_FIT_TILT_DEG, TiltCalibration  # noqa: E402
...
def test_zero_maps_to_zero():
    ...
    if cal.model == "cylinder":
        ...
        print(
            f"  theta_raw 0         -> {got:.2f} deg, inside the {floor:.2f} deg "
            f"resolution floor 2*atan(k)"
        )
    else:
        assert abs(got) < 1e-9, got
        print(
            f"  theta_raw 0         -> {got:.2e} deg (no constant term by construction)"
        )
...
def test_monotonic_and_bounded():
    ...
    print(
        f"  monotone & bounded  0-90 deg in -> [{ys.min():.2f}, {ys.max():.2f}] deg out"
    )
...
def test_refuses_non_monotonic_fit():
    ...
    try:
        ...
        print(
            "  bad fit rejected    non-monotonic correction raises instead of shipping"
        )
        return
...
def test_apply_preserves_major_axis():
    ...
    assert (
        minor < 95.0
    ), "correction should shrink the minor axis, raising the implied tilt"
    print(
        f"  apply()             major fixed at {major:.1f} px, minor 95.0 -> {minor:.2f} px"
    )
...
def test_clamps_beyond_fit_range():
    ...
    if cal.model == "cylinder":
        ...
        print(
            f"  extrapolation       exact by construction: {MAX_FIT_TILT_DEG:g} -> "
            f"{at_edge:.2f} deg, 115 -> {beyond:.2f} deg, both bounded"
        )
    else:
        assert abs(beyond - at_edge) < 1e-9, (beyond, at_edge)
        print(
            f"  extrapolation       clamped at {MAX_FIT_TILT_DEG:g} deg -> {at_edge:.2f} deg"
        )
...
def test_correction_reduces_tilt_error_on_a_model():
    ...
    print(
        f"  error reduction     median |tilt err| {before:.2f} -> {after:.2f} deg "
